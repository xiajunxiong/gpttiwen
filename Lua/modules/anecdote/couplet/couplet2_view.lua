CoupletView2 = CoupletView2 or DeclareView("CoupletView2", "anecdote/couplet2_view", Mod.Anecdote.Couplet2)

function CoupletView2:CoupletView2()
...
function CoupletView2:LoadCallback(param_t)
	self.data:DataReset(AnecdoteType.Couplet2, param_t.config or Config.world_couplet_two_auto, param_t.now_group)
	...
	AnecdoteData.Instance:CustomAutoHandle(AnecdoteType.Couplet2, BindTool.Bind1(self.OnAutoFunc, self))
...
function CoupletView2:FlushFinishWord()
...
function CoupletView2:AddDelayTimer()
	self.ObjTopBigList:SetActive(true)
	self.DownList.gameObject:SetActive(true)
end

--主线任务自动完成行为
function CoupletView2:OnAutoFunc()
	if self.data:GetIsFinish() then
		return
	end
	ViewMgr:CloseView(DialogTipsView)
	self.data:FastFinish()
	self:OnCompleteClick(CoupletView2)
end

-- 将需要填的字的mono存储起来
function CoupletView2:SetSelectMono()
...
-- 点击关闭
function CoupletView2:OnCloseClick()
	if self.data:GetIsFinish() then
		return
	end
	-- 主线过来的直接关闭不需要提示框
	if self.send_func then
		if self.close_func then
			...
			ViewMgr:CloseView(CoupletView2)
		end
		return
	end
	local param_t = {
...
function CoupletView2:CloseCallback()
	...
	if self.need_show_talk_id ~= -1 then
		if self.need_show_talk_id == self.data:GetFailTalkId() then
			...
			if self.data:GetNowGroup() >= self.data.GROUP_MAX then
				AnecdoteCtrl.Instance:SendTaskAction(AnecdoteType.Couplet2)
			else
				NpcDialogCtrl.Instance:NpcDialogView(
					{talk_id = self.need_show_talk_id},
					function ()
						ViewMgr:OpenView(CoupletView2)
					end)
			end
...
-- 点击完成按钮
function CoupletView2:OnCompleteClick()
	if self.data:GetIsFinish() then
		return
	end
	-- 判断是否已经回答完所有对联了
