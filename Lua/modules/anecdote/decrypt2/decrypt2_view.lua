Decrypt2View = Decrypt2View or DeclareView("Decrypt2View", "anecdote/decrypt2_view", Mod.Anecdote.Decrypt2)
--沙漠
VIEW_DECLARE_MASK(Decrypt2View, ViewMask.None)
function Decrypt2View:Decrypt2View()
	self.data = CoupletData.Instance
end

function Decrypt2View:LoadCallback()
	self.data:DataReset(AnecdoteType.Decrypt2, Config.decrypt_desert_auto)
...
-- 将需要填的字的mono存储起来
function Decrypt2View:SetSelectMono()
...
-- 点击关闭
function Decrypt2View:OnCloseClick()
	...
	local param_t = {
		...
		confirm = {
			func = function()
				...
				ViewMgr:CloseView(Decrypt2View)
			end
		}
	}
...
function Decrypt2View:CloseCallback()
...
-- 点击完成按钮
function Decrypt2View:OnCompleteClick()
	if self.data:GetIsFinish() then
		return
	end
