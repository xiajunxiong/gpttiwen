...
FishMessengerData = FishMessengerData or BaseClass()
function FishMessengerData:__init()
    if FishMessengerData.Instance ~= nil then
        Debuger.LogError("[FishMessengerData] attempt to create singleton twice!")
        return
    end
    FishMessengerData.Instance = self
    self:FishMessengerData()
end

function FishMessengerData:FishMessengerData()
    self.config = Config.fish_messenger_auto
...
function FishMessengerData:RemTime()
    return self.config_other.rem_time
end

function FishMessengerData:PlayTime()
    return self.config_other.play_time
end

function FishMessengerData:GetShowList()
    self.show_list = self.cards_list[GameMath.Ramdon(1, #self.cards_list)]
    return self.show_list
end

function FishMessengerData:StartReset()
    self.card_history = 0
    self.card_succ:Set({
        index = 0,
        num = 0,
...
function FishMessengerData:CountDownStart(index)
    if 0 >= self.count_down.value then
        self.count_down.value = index
    end
end

function FishMessengerData:CardShowed(index, value)
...
function FishMessengerData:CardShow(index, value)
    local value_pre = self.show_list[self.card_history] and self.show_list[self.card_history].value or 0
