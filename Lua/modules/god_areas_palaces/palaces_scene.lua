PalacesScene = PalacesScene or BaseClass(BaseScene)
local Input = UnityEngine.Input
local touch_helper = Game.SceneTouchHelper
function PalacesScene:__init(scene_id)
...
function PalacesScene:__delete()
	Runner.Instance:RemoveRunObj(self)
end

function PalacesScene:StartLoading()
	...
	SceneData.Instance:SetMode(scene_cfg.scene_type)

	local map_cfg = Config_scenelist[scene_cfg.mapid]
	local sn = "GXZJ_1" --map_cfg.map_res
	SceneMgr:LoadScene(sn,function(error)
			if self:Disposed() then
				return
			end
			if not StringIsEmpty(error) then
				Debuger.LogError("PalacesScene Load Scene failed===%s,ERROR=%s",sn,error)
			else
...
function PalacesScene:ResetCamera()
	local scene_cfg = SceneData.Instance:SceneConfig()
	...
	-- MainPlayer:BindVirtualCamera(vir_cam2)
...
--特殊逻辑换地毯
function PalacesScene:ChangeCarpet(id)
...
function PalacesScene:ChangeCarpet3(id)
...
function PalacesScene:ChangeCarpet2(id)
...
function PalacesScene:PosToServer(pos, mapid)
	--位置偏移,放大,四舍五入
	local scene_cfg = SceneData.Instance:SceneConfig()
...
function PalacesScene:GetPosHeight(x,z,y)
...
function PalacesScene:GetHeight(x, z)
	if self.terrain_collider then
		local ret = self.terrain_collider:GetHeight(x,z)
		return ret
	end
...
function PalacesScene.SamplePos(pos)
	return Game.TerrainCollider.SamplePos(pos)
end

function PalacesScene:Update()
	if MainPlayer ~= nil then
...
function PalacesScene:updateTouch()
...
function PalacesScene:GmWatch()
	return GmData.Instance.view_data.free_move == true
end
--检查自动三界
function PalacesScene:ResearchTaskAuto()
...
function PalacesScene:ResearchDailyDegree()
...
--检查家族主动邀请
function PalacesScene:ResearchGuildLetter()
...
function PalacesScene:onMainPlayerPosChange(player_pos)
...
function PalacesScene:SceneId()
	return self.scene_id
end
function PalacesScene:IsTouchMoving()
	return self.is_touch_moving
end
-- 强制终止指令输入
function PalacesScene:StopTouchMoving()
	self.is_touch_begin = false
end

function PalacesScene:CameraPosCheck()
	-- statements
end

function PalacesScene:SetMainVirtualCamera(scene_id,vir_cam_obj,vir_cam)
	vir_cam.Priority = 8
...
function PalacesScene:MainVirtualCameraAnimation(clipName,func)
...
function PalacesScene:ForceStopMainVirtualCameraAnimation()
    if self.camera_anim_handle then
        Runner.Instance:RemoveRunUntilTrue(self.camera_anim_handle)
        self.camera_anim_handle = nil
    end
...
PalacesScenePanel = PalacesScenePanel or DeclareMono("PalacesScenePanel", UIWFlushPanel)
function PalacesScenePanel:PalacesScenePanel()
	self.data = PalacesData.Instance
end
function PalacesScenePanel:OnEnable()
	UIWFlushPanel.OnEnable(self)
	self.data:SetSceneMono(self)
end
function PalacesScenePanel:OnDestroy()
    UIWFlushPanel.OnDestroy(self)
	self.data:SetSceneMono(nil)
end