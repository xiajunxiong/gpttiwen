RuinsScene = RuinsScene or BaseClass(BaseScene)
local Input = UnityEngine.Input
local touch_helper = Game.SceneTouchHelper
function RuinsScene:__init(scene_id)
...
function RuinsScene:__delete()
	Runner.Instance:RemoveRunObj(self)
end

function RuinsScene:StartLoading()
	...
	SceneMgr:LoadScene(sn,function(error)
			...
			if not StringIsEmpty(error) then
				Debuger.LogError("RuinsScene Load Scene failed===%s,ERROR=%s",sn,error)
			else
...
function RuinsScene:ResetCamera()
	local scene_cfg = SceneData.Instance:SceneConfig()
...
--特殊逻辑换地毯
function RuinsScene:ChangeCarpet(id)
...
function RuinsScene:ChangeCarpet3(id)
...
function RuinsScene:ChangeCarpet2(id)
...
function RuinsScene:PosToServer(pos, mapid)
	--位置偏移,放大,四舍五入
	local scene_cfg = SceneData.Instance:SceneConfig()
...
function RuinsScene:GetPosHeight(x,z,y)
...
function RuinsScene:GetHeight(x, z)
	if self.terrain_collider then
		local ret = self.terrain_collider:GetHeight(x,z)
		return ret
	end
...
function RuinsScene.SamplePos(pos)
	return Game.TerrainCollider.SamplePos(pos)
end

function RuinsScene:Update()
	if MainPlayer ~= nil then
...
function RuinsScene:updateTouch()
...
function RuinsScene:GmWatch()
	return GmData.Instance.view_data.free_move == true
end
--检查自动三界
function RuinsScene:ResearchTaskAuto()
...
function RuinsScene:ResearchDailyDegree()
...
--检查家族主动邀请
function RuinsScene:ResearchGuildLetter()
...
function RuinsScene:onMainPlayerPosChange(player_pos)
...
function RuinsScene:SceneId()
	return self.scene_id
end
function RuinsScene:IsTouchMoving()
	return self.is_touch_moving
end
-- 强制终止指令输入
function RuinsScene:StopTouchMoving()
	self.is_touch_begin = false
end

function RuinsScene:CameraPosCheck()
	-- statements
end

function RuinsScene:SetMainVirtualCamera(scene_id,vir_cam_obj,vir_cam)
	vir_cam.Priority = 8
...
function RuinsScene:MainVirtualCameraAnimation(clipName,func)
...
function RuinsScene:ForceStopMainVirtualCameraAnimation()
    if self.camera_anim_handle then
        Runner.Instance:RemoveRunUntilTrue(self.camera_anim_handle)
        self.camera_anim_handle = nil
    end
...
RuinsScenePanel = RuinsScenePanel or DeclareMono("RuinsScenePanel", UIWFlushPanel)
function RuinsScenePanel:RuinsScenePanel()
	self.data = RuinsData.Instance
end
function RuinsScenePanel:OnEnable()
	UIWFlushPanel.OnEnable(self)
	self.data:SetSceneMono(self)
end
function RuinsScenePanel:OnDestroy()
