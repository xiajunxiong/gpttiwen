QuickBuyNormal = QuickBuyNormal or DeclareView("QuickBuyNormal","item_info/quick_buy_normal")

function QuickBuyNormal:QuickBuyNormal()
    self.Board:SetData(self:GetType(),Language.ItemInfo.QuickBuy,Vector2.New(678, 500))
end

function QuickBuyNormal:LoadCallback(param_t)
    ...
    self.CellList:SetData(neo_list)
    ...
    for k,v in pairs(self.item_list) do 
        ...
        if is_limit or not is_require_limit then 
            ...
            ViewMgr:CloseView(QuickBuyNormal)
            return
        end 
    end 

    if ItemInfoCtrl.Instance:IsShowQuick(self.param_t.key) == true then 
        ViewMgr:CloseView(QuickBuyNormal)
        self:OnClickConfirm()
        return
    end 
    ...
    local desc = param_t.desc or Language.ItemInfo.QuickBuyDesc
    UH.SetText(self.Desc, string.format(desc, ColorStr(price_str, COLORSTR.Green3)))
end 

function QuickBuyNormal:CloseCallback()
...
-- 通过获取途径获取具体的购买信息
function QuickBuyNormal:CatchBuyWay(item)
    return ItemInfoData.Instance:CatchForgeBuyWay(item)
end

function QuickBuyNormal:OnClickConfirm()
    ...
    local need_gold_fun = function (buy_num)
        if not MallCtrl.IsNotCurrency(CurrencyType.Gold,buy_num) then
            PublicPopupCtrl.Instance:Center(Language.Role.PopupTips9)
            return true 
        end
        return false
    end
    ...
    for k,v in pairs(self.item_list) do 
        if (v.shop_data.price_type ~= nil and v.shop_data.price_type == CurrencyType.ImmortalCoin) or
        (v.shop_data.money_type ~= nil and v.shop_data.money_type == CurrencyType.ImmortalCoin) then 
            total_price = total_price + MarketData.Instance:GetItemPrice(v.info.item_id) * v.jump_need_num
        elseif (v.shop_data.money_type ~= nil and v.shop_data.money_type == CurrencyType.CoinBind) or
        (v.shop_data.price_type ~= nil and v.shop_data.price_type == CurrencyType.CoinBind) then 
            total_coin = total_coin + MarketData.Instance:GetItemPrice(v.info.item_id) * v.jump_need_num
        elseif (v.shop_data.money_type ~= nil and v.shop_data.money_type == CurrencyType.Gold) or
            (v.shop_data.price_type ~= nil and v.shop_data.price_type == CurrencyType.Gold) then 
                total_gold = total_gold + v.shop_data.price
        end 
    ...
    if total_gold > 0 and need_gold_fun(total_gold) then
        return
    end

    
    if self.param_t.auto_buy == true then
        for k,v in pairs(self.item_list) do 
            ...
            elseif v.mod == Mod.Store.Gold then
                ShoppingCtrl.Instance:SendStoreBuyReq(v.shop_data)
            end 
        end
    end

    if self.param_t.buy_func then
        self.param_t.buy_func()
    end
...
function QuickBuyNormal:OnClickTick(flag)
    ItemInfoData.Instance:SetQuickBuyTips(self.param_t.key, flag)
end 
