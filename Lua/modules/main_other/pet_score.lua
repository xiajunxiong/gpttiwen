--属性变化
PetScore = PetScore or DeclareView("PetScore", "main_other/pet_score", 0, ViewLayer.Top - 2)
VIEW_DECLARE_LEVEL(PetScore, ViewLevel.Lt)
VIEW_DECLARE_MASK(PetScore, ViewMask.None)

PetScorePanel = PetScorePanel or DeclareMono("PetScorePanel", UIWFlushPanel)
function PetScorePanel:PetScorePanel()
end
function PetScorePanel:onFlush()
    self.OpenAnim:Play()
    self.CloseAnim:Stop()
    --动画帧必须比停止帧长
    --24帧1秒
    ...
    --获取旧值
    local score, old_score = PetData.Instance:GetScoreChange()
...
function PetScorePanel:OnDestroy()
