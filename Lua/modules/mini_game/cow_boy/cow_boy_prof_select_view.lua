CowBoyProfSelectView = CowBoyProfSelectView or DeclareView("CowBoyProfSelectView", "mini_game/cow_boy_prof_select")
VIEW_DECLARE_LEVEL(CowBoyProfSelectView, ViewLevel.Ls)
VIEW_DECLARE_MASK(CowBoyProfSelectView, ViewMask.None)
function CowBoyProfSelectView:CowBoyProfSelectView()
    -- 小游戏创角界面,共用
...
function CowBoyProfSelectView:LoadCallback()
...
function CowBoyProfSelectView:GetRandName()
    self.rand_name = RoleCreateData.Instance.create_view_data.role_name
end

function CowBoyProfSelectView:SetRandInfo()
    self.rand_info = {}
...
function CowBoyProfSelectView:OnClickSureChara()
...
function CowBoyProfSelectView:CreateRole()
...
-- 创角成功后
function CowBoyProfSelectView:CreateSuccess()
    ...
    ViewMgr:CloseView(CowBoyProfSelectView)
end

function CowBoyProfSelectView:FlushSkillPanel()
    if self.Data.prof_view_data.is_show_skill_tips then
        LogError("SelectId = ",self.SelectId)
        UH.AnchoredPosition(self.SkillPanelRect, Vector2.New(self.clicked_detail == 1 and 60 or -103, 54))
    end
...
function CowBoyProfSelectView:CloseSkillPanel()
    NeoCharaChangeData.Instance:SetProfViewSkill({is_show = false})
end 

function CowBoyProfSelectView:FlushAll()
    ...
    UH.SpriteName(self.CurName, "_LocZhiYeWenZi"..cur_prof)
    ...
    UH.SpriteName(self.NextName, "_LocZhiYeWenZi"..next_prof)
end

function CowBoyProfSelectView:FlushSelect(index)
    self.SelectId = index
    self.BgSel1:SetActive(index == 1)
    self.BgSel2:SetActive(index == 2)
    self.Bg1:SetActive(index == 2)
    self.Bg2:SetActive(index == 1)
end

function CowBoyProfSelectView:ClickCurToggleOn()
    self.CurBtnToggleOn:SetActive(false)
    self.CurProfToggleAnim:SetTrigger(APH("is_show"))
    if self.clicked_detail == 2 then
        self:ClickNextToggleOff()
    end
    self.clicked_detail = 1
end

function CowBoyProfSelectView:ClickCurToggleOff()
    self.clicked_detail = nil
    self.CurBtnToggleOn:SetActive(true)
    self.CurProfToggleAnim:SetTrigger(APH("is_hide"))
end

function CowBoyProfSelectView:ClickNextToggleOn()
    self.NextBtnToggleOn:SetActive(false)
    self.NextProfToggleAnim:SetTrigger(APH("is_show"))
    if self.clicked_detail == 1 then
        self:ClickCurToggleOff()
    end
    self.clicked_detail = 2
end

function CowBoyProfSelectView:ClickNextToggleOff()
    self.clicked_detail = nil
    self.NextBtnToggleOn:SetActive(true)
    self.NextProfToggleAnim:SetTrigger(APH("is_hide"))
end

function CowBoyProfSelectView:CloseCallback()
    if self.cur_ui_obj ~= nil then
