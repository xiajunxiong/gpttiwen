...
ChallengeTestNotice = ChallengeTestNotice or DeclareView("ChallengeTestNotice", "notice/challenge_test_notice", Mod.FunOpenNotice.ChallengeTest)

VIEW_DECLARE_LEVEL(ChallengeTestNotice, ViewLevel.L1)

ChallengeTestNoticeF = ChallengeTestNoticeF or DeclareMono("ChallengeTestNoticeF", UIWFlushPanel)

function ChallengeTestNoticeF:onFlush()
    self.config = GuideData.Instance:GetFunctionNoticeData(Mod.FunOpenNotice.ChallengeTest)
...
function ChallengeTestNoticeF:OnClickClose()
    ViewMgr:CloseView(ChallengeTestNotice)
end

function ChallengeTestNoticeF:OnClickGet()
    local flag = GuideData.Instance:NoticeInfo(self.config.notice_id)
...
-- 规则：任务完成后显示开启奖励
function ChallengeTestNoticeF:FlushGuideUnit()
