ModronsNotice = ModronsNotice or DeclareView("ModronsNotice", "notice/modrons_notice", Mod.FunOpenNotice.Modrons)
VIEW_DECLARE_LEVEL(ModronsNotice, ViewLevel.L1)
ModronsNoticePanel = ModronsNoticePanel or DeclareMono("ModronsNoticePanel", UIWFlushPanel)
function ModronsNoticePanel:onFlush()
    self.config = GuideData.Instance:GetFunctionNoticeData(Mod.FunOpenNotice.Modrons)
...
function ModronsNoticePanel:OnClickClose()
    ViewMgr:CloseView(ModronsNotice)
end
function ModronsNoticePanel:OnClickGet()
    local flag = GuideData.Instance:NoticeInfo(self.config.notice_id)
    ...
    elseif flag == FunPreNoticeEnum.UnLock then
        ...
        ViewMgr:OpenViewByKey(Mod.Modrons.Main) 
        ViewMgr:CloseView(FunOpenNoticeView)
    end
end

-- 规则：任务完成后显示开启奖励
function ModronsNoticePanel:FlushGuideUnit()
