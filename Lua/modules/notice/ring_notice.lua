RingNotice = RingNotice or DeclareView("RingNotice", "notice/ring_notice", Mod.FunOpenNotice.Ring)
VIEW_DECLARE_LEVEL(RingNotice, ViewLevel.L1)

RingNoticePanel = RingNoticePanel or DeclareMono("RingNoticePanel", UIWFlushPanel)

function RingNoticePanel:onFlush()
    self.config = GuideData.Instance:GetFunctionNoticeData(Mod.FunOpenNotice.Ring)
...
function RingNoticePanel:OnClickClose()
    ViewMgr:CloseView(RingNotice)
end

function RingNoticePanel:OnClickGet()
    local flag = GuideData.Instance:NoticeInfo(self.config.notice_id)
...
-- 规则：任务完成后显示开启奖励
function RingNoticePanel:FlushGuideUnit()
