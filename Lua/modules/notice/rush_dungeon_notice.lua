RushDungeonNotice = RushDungeonNotice or DeclareView("RushDungeonNotice", "notice/rush_dungeon_notice", Mod.FunOpenNotice.RushDungeon)
VIEW_DECLARE_LEVEL(RushDungeonNotice, ViewLevel.L1)
RushDungeonNoticePanel = RushDungeonNoticePanel or DeclareMono("RushDungeonNoticePanel", UIWFlushPanel)
function RushDungeonNoticePanel:onFlush()
    self.config = GuideData.Instance:GetFunctionNoticeData(Mod.FunOpenNotice.RushDungeon)
...
function RushDungeonNoticePanel:OnClickClose()
    ViewMgr:CloseView(RushDungeonNotice)
end
function RushDungeonNoticePanel:OnClickGet()
    local flag = GuideData.Instance:NoticeInfo(self.config.notice_id)
    ...
    elseif flag == FunPreNoticeEnum.UnLock then
        ...
        ViewMgr:OpenViewByKey(Mod.Challenge.RushDungeon)
        ViewMgr:CloseView(FunOpenNoticeView)
    end
end

-- 规则：任务完成后显示开启奖励
function RushDungeonNoticePanel:FlushGuideUnit()
