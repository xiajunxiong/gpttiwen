...
SanXianGuiDongData = SanXianGuiDongData or BaseClass()
function SanXianGuiDongData:__init()
    if SanXianGuiDongData.Instance ~= nil then
        Debuger.LogError('[SanXianGuiDongData] attempt to create singleton twice!')
        return
    end
    SanXianGuiDongData.Instance = self
    self.cfg_info = Config.sanxian_game_auto
...
function SanXianGuiDongData:InitConfig()
...
function SanXianGuiDongData:__delete()
end
function SanXianGuiDongData:Clear()
...
function SanXianGuiDongData:ActInfo()
    return self.info
end
function SanXianGuiDongData:Game()
    return self.game
end
function SanXianGuiDongData:StartTime()
    return self.other_info.start_time
end

function SanXianGuiDongData:CurLevel()
    return self.game.pass_idx
end
function SanXianGuiDongData:RewardLevel()
...
function SanXianGuiDongData:ChangeList()
    local cur_level = self:CurLevel()
...
function SanXianGuiDongData:LevelCfg(cur_level)
    cur_level = cur_level or self:CurLevel()
    local level_cfg = self.level_cfg[cur_level]
    return level_cfg
end
function SanXianGuiDongData:NotifyStart()
    self.change.start = not self.change.start
end
function SanXianGuiDongData:RightIdxs(need, max)
    local list = {}
    for i = 1, need do
...
function SanXianGuiDongData:RamdonNum(max, list)
...
function SanXianGuiDongData:FinalArray(list, list2)
    local a, b
...
function SanXianGuiDongData:ChangePos(a, b)
    a = tonumber(a)
    b = tonumber(b)
    a = a + b
    b = a - b
...
function SanXianGuiDongData:NeedRabbit(unit_num)
    local num = 0
    for i, v in ipairs(self.level_cfg) do
...
function SanXianGuiDongData:SetPosObj(objs)
    self.pos_objs = objs
end
function SanXianGuiDongData:GetPosObj()
    return self.pos_objs
end
function SanXianGuiDongData:NotifyInit()
    self.change.init = not self.change.init
end

function SanXianGuiDongData:SetRightIdxs(idxs)
    self.right_idxs = idxs
end
function SanXianGuiDongData:GetRightIdxs()
    return self.right_idxs
end
function SanXianGuiDongData:IsChangeMode(pass_idx)
    local old = self:LevelCfg(pass_idx - 1)
    if old then
...
function SanXianGuiDongData:SelectIdxs()
    return self.select_idxs
end
function SanXianGuiDongData:SetSelect(index)
    table.insert(self.select_idxs, index)
end
function SanXianGuiDongData:ClearSelect()
    self.select_idxs = {}
end
function SanXianGuiDongData:GetPassReward()
...
function SanXianGuiDongData:ShowReward()
    if self:IsOutCount() then
        return {}
    end
    local pass_level = self.final_pass_idx
...
function SanXianGuiDongData:PassRewardDesc()
    if self:IsOutCount() then
        return ""
    end
    local pass_level = self.final_pass_idx
...
function SanXianGuiDongData:IsPassFinal()
    return self.final_pass_idx == self.other_info.max_level
end
function SanXianGuiDongData:IsMaxLevel(pass_idx)
    return pass_idx > self.other_info.max_level
end
function SanXianGuiDongData:IsActiveTitle()
