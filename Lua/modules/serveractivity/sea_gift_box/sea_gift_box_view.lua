SeaGiftBoxView = SeaGiftBoxView or DeclareView("SeaGiftBoxView", "serveractivity/sea_gift_box_view", Mod.MountainSea.SeaGiftBox)
VIEW_DECLARE_LEVEL(SeaGiftBoxView, ViewLevel.L0)

function SeaGiftBoxView:LoadCallback(param_t)
    UH.SetText(self.Title, ActivityRandData.GetName(ACTIVITY_TYPE.RAND_SEA_GIFT_BOX))
end

function SeaGiftBoxView:OnClickClose()
    ViewMgr:CloseView(SeaGiftBoxView)
end

SeaGiftBoxPanel = SeaGiftBoxPanel or DeclareMono("SeaGiftBoxPanel", UIWFlushPanel)
function SeaGiftBoxPanel:SeaGiftBoxPanel()
    self.cur_page, self.min_page, self.max_page = 1, 1, 1
    self.is_ordering = false
    self.data_cares = {
        {data = SeaGiftBoxData.Instance.gift_box_data, func = self.onFlush},
...
function SeaGiftBoxPanel:Awake()
	UIWFlushPanel.Awake(self)
    self.Timer:CreateActTime(ACTIVITY_TYPE.RAND_SEA_GIFT_BOX, TimeType.Type_Time_3, Language.RAUstomizedBox.Txt2)
end

function SeaGiftBoxPanel:OnDestroy()
...
function SeaGiftBoxPanel:onFlush()
    self.is_ordering = false
    local base_list = SeaGiftBoxData.Instance:GetGiftBoxShowList()
...
function SeaGiftBoxPanel:FlushPageItem()
...
function SeaGiftBoxPanel:OnClickPage(param)
...
function SeaGiftBoxPanel:DoOrdering()
    self.is_ordering = true
    self:FlushPageItem()
end
function SeaGiftBoxPanel:IsOrdering()
    return self.is_ordering
end

function SeaGiftBoxPanel:SendBuy()
    ServerActivityCtrl.Instance:MoneyBuy(BUY_ITEM_TYPE.TYPE_SEA_GIFT_BOX, self.buy_money)
end

function SeaGiftBoxPanel:SaveMoney(buy_money)
    self.buy_money = buy_money
end

function SeaGiftBoxPanel:FlushFreeGift()
    self.FreeGiftNode:SetActive(not SeaGiftBoxData.Instance:IsFreeGiftFetched())
end

function SeaGiftBoxPanel:OnClickFreeGift()
    ServerActivityCtrl.Instance:SendActivityReq(ACTIVITY_TYPE.RAND_SEA_GIFT_BOX, 2)
end

SeaGiftBoxItem = SeaGiftBoxItem or DeclareMono("SeaGiftBoxItem", UIWidgetBaseItem)
function SeaGiftBoxItem:SeaGiftBoxItem()
    self.lang = Language.RAGiftBox
    self.effect_id = 0
end

function SeaGiftBoxItem:SetData(data)
    UIWidgetBaseItem.SetData(self, data)
    self:FlushAllInfo()
end

function SeaGiftBoxItem:FlushAllInfo()
    UH.SetText(self.NameText, self.data.gift_des)
    if self.data.limit_type == 1 then
        local buy_times = SeaGiftBoxData.Instance:GetGiftBoxBuyTimes(self.data.seq)
        local max_times = self.data.buy_times
        self:TimesStr(self.lang.Limit1, buy_times, max_times)
    else
        local buy_times = SeaGiftBoxData.Instance:GetGiftBoxDayBuyTimes(self.data.seq)
    ...
    local have_times = SeaGiftBoxData.Instance:IsGiftBoxBuyHaveTimes(self.data)
    if have_times then
...
function SeaGiftBoxItem:ShowItems()
    local show_list = {}
...
function SeaGiftBoxItem:MoneyPrice()
    return RechargeData.Instance:GoldToMoney(self.data.buy_money)
end

function SeaGiftBoxItem:OnClickBuy()
    if not SeaGiftBoxData.Instance:IsGiftBoxBuyHaveTimes(self.data) then
    ...
    ServerActivityCtrl.Instance:SendBuyCmd(BUY_ITEM_TYPE.TYPE_SEA_GIFT_BOX,fee,ACTIVITY_TYPE.RAND_SEA_GIFT_BOX, 1, self.data.seq, 0, 0)

    self.BtnBuyInter.Interactable = false
    self.BoxPanel:DoOrdering()
end

function SeaGiftBoxItem:TimesStr(label, buy_times, max_times)
...
function SeaGiftBoxItem:ShowEffect()
...
function SeaGiftBoxItem:GetEffectId(icon_type)
    if icon_type == 1 then
        return 100108155
    else
