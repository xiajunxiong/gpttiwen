--属性变化
ShengQiScore = ShengQiScore or DeclareView("ShengQiScore", "sheng_qi/part/sheng_qi_score", 0, ViewLayer.Top - 2)
VIEW_DECLARE_LEVEL(ShengQiScore, ViewLevel.Lt)
VIEW_DECLARE_MASK(ShengQiScore, ViewMask.None)

ShengQiScorePanel = ShengQiScorePanel or DeclareMono("ShengQiScorePanel", UIWFlushPanel)
function ShengQiScorePanel:ShengQiScorePanel()
end

function ShengQiScorePanel:Awake()
    UIWFlushPanel.Awake(self)
    Invoke(function ()
        if ViewMgr:IsOpen(ShengQiScore) then
            ViewMgr:CloseView(ShengQiScore)
        end
    end,4)
    self:FlushPanel()
end
function ShengQiScorePanel:FlushPanel()
    self.OpenAnim:Play()
    self.CloseAnim:Stop()
    --动画帧必须比停止帧长
    --24帧1秒
...
function ShengQiScorePanel:OnDestroy()
