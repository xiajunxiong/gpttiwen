StarTreasureMapRollerView = StarTreasureMapRollerView or DeclareView("StarTreasureMapRollerView", "star_treasure_map/star_treasure_map_roller")

VIEW_DECLARE_LEVEL(StarTreasureMapRollerView,ViewLevel.L1)

function StarTreasureMapRollerView:StarTreasureMapRollerView()
...
function StarTreasureMapRollerView:CloseCallback()
    TimeHelper:CancelTimer(self.ShowTimer)
    TimeHelper:CancelTimer(self.AnimTimer)
    StarTreasureMapData.Instance:NextStarTreasureMap()
    TimeHelper:CancelTimer(self.auto_time_ht)
end

function StarTreasureMapRollerView:OnClickClose()
    ViewMgr:CloseView(StarTreasureMapRollerView)
end

function StarTreasureMapRollerView:LoadCallback(param_t)
    local config = StarTreasureMapData.Instance:GetConfigData()
    ...
    if is_auto then
        self.auto_time_ht = Invoke(function ()
            local _is_auto = StarTreasureMapData.Instance:GetAutoFlag()
            if _is_auto then
                self.BtnEnter.localPosition = Vector3.zero
...
--=========================动画逻辑=========================
function StarTreasureMapRollerView:OnPlayAnimation()
...
function StarTreasureMapRollerView:UpdataRollerTime()
...
function StarTreasureMapRollerView:FlushCellListView(value)
...
function StarTreasureMapRollerView:FlushEndView()
    PublicPopupCtrl.Instance:ShowCustom({Item.New(self.param_t.reward)})
    local use_item_id = TreasureMapData.Instance:GetStarTreasureMapId()
...
--=========================拖拽逻辑=========================
function StarTreasureMapRollerView:OnDragBegin()
...
function StarTreasureMapRollerView:OnDrag()
    if self.IsOnPlayAnimation then return end
    Runner.Instance:RemoveRunUntilTrue(self.time_handle)
...
function StarTreasureMapRollerView:OnDragEnd()
    if self.IsOnPlayAnimation then return end
    Runner.Instance:RemoveRunUntilTrue(self.time_handle)
