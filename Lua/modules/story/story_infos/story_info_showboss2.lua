...
StoryInfoShowBoss2 = StoryInfoShowBoss2 or BaseClass(StoryInfo)

function StoryInfoShowBoss2:StoryInfo()
    self.trigger_type = StoryConfig.TriggerType.BattleStart
    self.ready_time = 1
end

function StoryInfoShowBoss2:StoryReady()
    -- BattleData.Instance:BaseData().pause = true
    ViewMgr:MainViewVisible(false)
    ViewMgr:OpenView(StoryShowBossView)
end

function StoryInfoShowBoss2:StoryLoaded(obj)
...
function StoryInfoShowBoss2:StoryDyLoad(wait_until_get_resource, res_type, res_id)
...
function StoryInfoShowBoss2:NeedTrigger()
