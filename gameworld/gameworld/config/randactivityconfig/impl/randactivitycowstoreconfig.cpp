#include "randactivitycowstoreconfig.hpp"
#include "servercommon/activitydef.hpp"

RandActivityCowStoreConfig::RandActivityCowStoreConfig()
{
}

RandActivityCowStoreConfig::~RandActivityCowStoreConfig()
{
	for (std::map<long long, std::map<long long, std::vector<RACowStoreCfg> > >::iterator it = m_section_map.begin(); it!= m_section_map.end(); it++){
		for (std::map<long long, std::vector<RACowStoreCfg> >::iterator node_it = it->second.begin(); node_it != it->second.end(); node_it++){
			std::vector<RACowStoreCfg>().swap(node_it->second);
		}
	}
}

bool RandActivityCowStoreConfig::Init(TiXmlElement *RootElement, std::string *err)
{
	PRE_LOAD_RA_CONFIG;

	LOAD_RA_CONFIG("gift_configure", InitCowStoreCfg);

	return true;
}

const RACowStoreCfg * RandActivityCowStoreConfig::GetCowStoreCfgByIndex(int id) const
{
	std::map<int, RACowStoreCfg>::const_iterator it = m_index_map.find(id);
...
const RACowStoreCfg * RandActivityCowStoreConfig::GetCowStoreCfgByActivityDay(int server_open_day, int open_day, int id) const
{
	for (std::map<long long, std::map<long long, std::vector<RACowStoreCfg> > >::const_iterator begin_it = m_section_map.begin(); begin_it != m_section_map.end(); begin_it++)
	{
		int time_start = 0;
		int time_end = 0;
		...
		{
			for (std::map<long long, std::vector<RACowStoreCfg> >::const_iterator it = begin_it->second.begin(); it != begin_it->second.end(); it++)
			{
				int section_start = 0;
				int section_end = 0;
				...
				{
					for (std::vector<RACowStoreCfg>::const_iterator itor = it->second.begin(); itor != it->second.end(); itor++)
					{
						if (itor->seq == id)
						{
							return &(*itor);
...
int RandActivityCowStoreConfig::InitCowStoreCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		RACowStoreCfg &node_cfg = m_index_map[type];
		node_cfg.type = type;
		...
		if (!GetSubNodeValue(data_element, "price_type", node_cfg.price_type) || node_cfg.price_type < RACowStoreCfg::PRICE_TYPE_GOLD ||
			node_cfg.price_type > RACowStoreCfg::PRICE_TYPE_MONEY) return -6;
...
long long RandActivityCowStoreConfig::GetKeyBySection(int section_start, int section_end) const
{
	KeyChangeUn un_key;
...
void RandActivityCowStoreConfig::GetSectionByKey(long long key, int * section_start, int * section_end) const
{
	KeyChangeUn un_key;
	un_key.key = key;
