...
#include "randactivitydingzhibaoxiang2config.hpp"

RandActivityDingZhiBaoXiang2Config::RandActivityDingZhiBaoXiang2Config()
{
}

RandActivityDingZhiBaoXiang2Config::~RandActivityDingZhiBaoXiang2Config()
{
	std::vector<RADingZhiBaoXiang2SectionCfg>().swap(m_section_cfg);
}

bool RandActivityDingZhiBaoXiang2Config::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;
...
const RADingZhiBaoXiang2ItemCfg * RandActivityDingZhiBaoXiang2Config::GetRADingZhiBaoXiangItemCfg(RandActivityManager * ramgr, int seq) const
{
	if (seq < 0 || seq >= RA_DING_ZHI_BAO_XIANG_2_MAX_ITEM) return NULL;

	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_DING_ZHI_BAO_XIANG_2);
	for (std::vector<RADingZhiBaoXiang2SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
const RADingZhiBaoXiang2RewardGroup * RandActivityDingZhiBaoXiang2Config::GetRADingZhiBaoXiangRewardGroup(int reward_group_id) const
{
	std::map<int, RADingZhiBaoXiang2RewardGroup>::const_iterator iter = m_reward_group.find(reward_group_id);
...
int RandActivityDingZhiBaoXiang2Config::GetRADingZhiBaoXiangItemNum(RandActivityManager * ramgr) const
{
	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_DING_ZHI_BAO_XIANG_2);
	for (std::vector<RADingZhiBaoXiang2SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityDingZhiBaoXiang2Config::InitBuyCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_section_cfg.size() > 0)
		{
			    ...
			    section_end != m_section_cfg[pre_index].section_end)
			{
				...
				next_seq = 0;
				m_section_cfg.push_back(RADingZhiBaoXiang2SectionCfg());
			}
		}
		if (m_section_cfg.size() == 0)
		{
		...
		RADingZhiBaoXiang2SectionCfg & node_cfg = m_section_cfg[m_section_cfg.size() - 1];
		node_cfg.section_start = section_start;
		node_cfg.section_end = section_end;

		RADingZhiBaoXiang2ItemCfg item_cfg;
		if (!GetSubNodeValue(data_element, "seq", item_cfg.seq) || item_cfg.seq != next_seq || item_cfg.seq >= RA_DING_ZHI_BAO_XIANG_2_MAX_ITEM) return -__LINE__;
...
int RandActivityDingZhiBaoXiang2Config::InitRewardGroupCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		RADingZhiBaoXiang2RewardGroup & item_cfg = m_reward_group[group_id];
...
int RandActivityDingZhiBaoXiang2Config::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
