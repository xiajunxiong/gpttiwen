#include "randactivityfateentanglementconfig.hpp"
...
RandActivityFateEntanglementConfig::RandActivityFateEntanglementConfig()
	:m_task_rate_count(0)
{
...
RandActivityFateEntanglementConfig::~RandActivityFateEntanglementConfig()
{
	std::vector<RAFateTaskRewardSectionSectionCfg>().swap(m_reward_section_section_cfg);
}

bool RandActivityFateEntanglementConfig::Init(TiXmlElement *RootElement, std::string *err)
{
	PRE_LOAD_RA_CONFIG;
...
int RandActivityFateEntanglementConfig::GetOpenDay(RandActivityManager * ramgr) const
{
	if (NULL == ramgr) return 1;

	int act_real_open_day = this->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_FATE_ENTANGLEMENT);
	int act_open_day = this->GetActOpenDay(ramgr, RAND_ACTIVITY_TYPE_FATE_ENTANGLEMENT, true);
...
int RandActivityFateEntanglementConfig::InitTaskRewardCfg(TiXmlElement *RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		{
			m_reward_section_section_cfg.push_back(RAFateTaskRewardSectionSectionCfg(start_time, end_time));
		...
		RAFateTaskRewardSectionSectionCfg & section_cfg = m_reward_section_section_cfg.back();

		std::vector<RAFateTaskRewardSectionCfg>& m_reward_section_cfg = m_reward_section_section_cfg.rbegin()->m_reward_section_cfg;
		...
		if (m_reward_section_cfg.size() > 0)
		{
				...
				section_end != m_reward_section_cfg[pre_index].section_end)
			{
				...
				m_reward_section_cfg.push_back(RAFateTaskRewardSectionCfg());
			}
		}
		if (m_reward_section_cfg.size() == 0)
		{
		...
		RAFateTaskRewardSectionCfg & node_cfg = m_reward_section_cfg[m_reward_section_cfg.size() - 1];
		...
		RAFateTaskRewardCfg reward_cfg;
...
int RandActivityFateEntanglementConfig::InitRewardGroupCfg(TiXmlElement  *RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
...
int RandActivityFateEntanglementConfig::InitTaskCfg(TiXmlElement *RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		RAFateTaskInfoCfg info;
		if (!GetSubNodeValue(data_element, "task_type", info.task_type) || info.task_type < FATE_ENTANGLEMENT_TYPE_BEGIN || info.task_type > FATE_ENTANGLEMENT_TYPE_MAX) return -1;
		if (FATE_ENTANGLEMENT_TYPE_BEGIN == info.task_type)	//初始任务不读
		{
			data_element = data_element->NextSiblingElement();
			continue;
		}
		...
		{
		case FATE_ENTANGLEMENT_TYPE_DELIVERY:
		case FATE_ENTANGLEMENT_TYPE_INSTEAD_OF_SEND:
		{
			if (NULL == ITEMPOOL->GetItem(info.param1) || info.param2 <= 0) return -(10 + info.task_type);
		}
		break;
		case FATE_ENTANGLEMENT_TYPE_BATTLE:
		{
			if (info.param2 <= 0) return -(10 + info.task_type);
		}
		break;
		case FATE_ENTANGLEMENT_TYPE_DELIVERY_PET:
		{
			if (info.param2 <= 0) return -(10 + info.task_type);
...
int RandActivityFateEntanglementConfig::InitOtherCfg(TiXmlElement *RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
...
const RAFateTaskInfoCfg * RandActivityFateEntanglementConfig::GetTaskInfoCfg(int task_id)const
{
	std::map<int, RAFateTaskInfoCfg>::const_iterator it = m_task_map.find(task_id);
	if (it != m_task_map.end()) return &it->second;

	return NULL;
}

const RAFateTaskInfoCfg * RandActivityFateEntanglementConfig::GetRankTaskCfg(int cur_task_id, bool is_can_rand_pet_task, bool is_first_rank_day)const
{
	if (is_first_rank_day)
	{
		int rand_rate = rand() % m_task_rate_count;
		for (std::map<int, RAFateTaskInfoCfg>::const_iterator it = m_task_map.begin(); it != m_task_map.end(); it++)
		{
	...
	{
		std::vector<const RAFateTaskInfoCfg *> snap_task_list;
		int snap_rate_count = 0;
		for (std::map<int, RAFateTaskInfoCfg>::const_iterator it = m_task_map.begin(); it != m_task_map.end(); it++)
		{
			if (cur_task_id != it->second.task_id && (is_can_rand_pet_task || FATE_ENTANGLEMENT_TYPE_DELIVERY_PET != it->second.task_type))
			{
...
const RAFateTaskRewardCfg * RandActivityFateEntanglementConfig::GetTaskRewardCfg(RandActivityManager* ramgr, int pass_count)const
{
	if (NULL == ramgr || pass_count <= 0 || pass_count > m_other_cfg.ring_up) return NULL;

	int act_open_day = this->GetActOpenDay(ramgr, RAND_ACTIVITY_TYPE_FATE_ENTANGLEMENT);
	int act_real_open_day = this->GetOpenDay(ramgr);

	for (std::vector<RAFateTaskRewardSectionSectionCfg>::const_iterator section_it = m_reward_section_section_cfg.begin(); m_reward_section_section_cfg.end() != section_it; ++section_it)
	{
		if (section_it->end_time >= act_real_open_day && act_real_open_day >= section_it->start_time)
		{
			for (int i = 0; i < (int)section_it->m_reward_section_cfg.size(); i++)
			{
				if (section_it->m_reward_section_cfg[i].section_start <= act_open_day && act_open_day <= section_it->m_reward_section_cfg[i].section_end)
				{
					...
					{
						const RAFateTaskRewardCfg & node = section_it->m_reward_section_cfg[i].node_list[k];
...
const RAFateRewardCfg * RandActivityFateEntanglementConfig::GetRewardCfg(int group_id)const
{
	std::map<int, RAFateRewardGroupCfg>::const_iterator it = m_reward_group_cfg.find(group_id);
	...
	{
		const RAFateRewardCfg & node = it->second.node_list[i];
