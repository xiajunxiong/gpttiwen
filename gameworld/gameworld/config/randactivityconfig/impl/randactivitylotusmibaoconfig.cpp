#include "randactivitylotusmibaoconfig.hpp"

#include "item/itempool.h"

RandActivityLotusMiBaoConfig::RandActivityLotusMiBaoConfig()
{
}

RandActivityLotusMiBaoConfig::~RandActivityLotusMiBaoConfig()
{
	std::vector<RALotusMiBaoSectionCfg>().swap(m_base_section_cfg);
}

bool RandActivityLotusMiBaoConfig::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;

	LOAD_RA_CONFIG("lotus_mibao", InitBaseCfg);
	LOAD_RA_CONFIG("lotus_raward_group", InitRewardGroupCfg);
	LOAD_RA_CONFIG("reward_configuration", InitFinishCondRewardCfg);	//需要放在奖励组读取之后
	
	return true;
}

const RALotusMiBaoCfg * RandActivityLotusMiBaoConfig::GetBaseCfg(RandActivityManager * ramgr, int row, int column) const
{
...
const RALotusMiBaoRewardCfg * RandActivityLotusMiBaoConfig::GetRandRewardCfg(int reward_group_id) const
{
	std::map<int, RALotusMiBaoRewardGroupCfg>::const_iterator it = m_reward_group_map.find(reward_group_id);
...
const RALotsMiBaoFinishCondRewardCfg * RandActivityLotusMiBaoConfig::GetCondRewardCfg(int cond_id, int param) const
{
	std::map<int, std::map<int, RALotsMiBaoFinishCondRewardCfg> >::const_iterator cond_it = m_finish_cond_reward_map.find(cond_id);
	if(cond_it == m_finish_cond_reward_map.end()) return NULL;

	std::map<int, RALotsMiBaoFinishCondRewardCfg>::const_iterator reward_it = cond_it->second.find(param);
	if(reward_it == cond_it->second.end()) return NULL;

	return &reward_it->second;
}

int RandActivityLotusMiBaoConfig::InitBaseCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_base_section_cfg.size() > 0)
		{
				...
				section_end != m_base_section_cfg[pre_index].section_end)
			{
				...
				next_row = 1;
				next_column = 0;
				m_base_section_cfg.push_back(RALotusMiBaoSectionCfg());
			}
		}
		if (m_base_section_cfg.size() == 0)
		{
		...
		RALotusMiBaoSectionCfg & node_cfg = m_base_section_cfg[m_base_section_cfg.size() - 1];
...
int RandActivityLotusMiBaoConfig::InitRewardGroupCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		RALotusMiBaoRewardCfg node_cfg;
...
int RandActivityLotusMiBaoConfig::InitFinishCondRewardCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		RALotsMiBaoFinishCondRewardCfg node_cfg;
