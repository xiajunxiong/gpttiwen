#include "randactivitymijingqiwenconfig2.hpp"
#include "item/itempool.h"
#include "servercommon/rolecrystalfbdef.h"

RandActivityMiJingQiWenConfig2::RandActivityMiJingQiWenConfig2()
{
}
...
RandActivityMiJingQiWenConfig2::~RandActivityMiJingQiWenConfig2()
{
	...
	for (std::map<int, std::vector<new_Event_NPC_cfg2> >::iterator it = m_event_npc_cfg_map.begin(); it != m_event_npc_cfg_map.end(); it++)
	{
		std::vector<new_Event_NPC_cfg2>().swap(it->second);
	}
	...
	std::vector<MiJingQiWenFbLevelTypeCfg2>().swap(m_level_to_seq_vec);
}

bool RandActivityMiJingQiWenConfig2::Init(TiXmlElement *RootElement, std::string *err)
{
	PRE_LOAD_RA_CONFIG;
...
const RoleCrystalFbLevelCfg * RandActivityMiJingQiWenConfig2::GetRoleCrystalFbLevelCfg(int seq) const
{
...
const int RandActivityMiJingQiWenConfig2::GetLayerNum(int seq)const
{
	if (seq < 0 || seq >= (int)m_level_cfg.size())
	{
		return 0;
...
const bool RandActivityMiJingQiWenConfig2::IsCrystalFbBoxGather(int gather_id)const
{
...
const CrystalFbBoxCfg::ItemCfg * RandActivityMiJingQiWenConfig2::GetBoxCfg(int group_id, int box_seq)const
{
...
//int RandActivityMiJingQiWenConfig2::GetRandScene(int scene_group_id, std::set<int>  exclude_scene_id)
//{
...
const CrystalFbSceneCfg::ItemCfg *  RandActivityMiJingQiWenConfig2::GetRandSceneCfg(int scene_group_id, std::set<int> exclude_scene_id) const
{
...
const Posi RandActivityMiJingQiWenConfig2::GetRandPosi(int pos_group_id)const
{
...
const CrystalFbBoxCfg::ItemCfg * RandActivityMiJingQiWenConfig2::GetRandBoxCfg(int rand_group_id)const
{
...
const std::vector<ItemConfigData>  RandActivityMiJingQiWenConfig2::GetRandBoxReward(std::vector<int> reward_group_ids)const
{
...
const ItemConfigData * RandActivityMiJingQiWenConfig2::GetRandNpcReward(int reward_group_ids)const
{
...
const int RandActivityMiJingQiWenConfig2::GetMaxRewardNum(int reward_group_id)const
{
...
const int RandActivityMiJingQiWenConfig2::GetRandNum(int num_group_id)const
{
...
const MiJingQiWenOtherCfg2 & RandActivityMiJingQiWenConfig2::GetCrystalFbOtherCfg()const
{
	return m_other_cfg;
}

const int RandActivityMiJingQiWenConfig2::GetUseItemLimitTimes(int use_limit_group_id, int use_type)const
{
...
const int RandActivityMiJingQiWenConfig2::IsLimitTimesUseType(int use_limit_group_id, int use_type)const
{
	std::map<int, CrystalFbUseItemLimitCfg>::const_iterator iter = m_use_item_limit_cfg.find(use_limit_group_id);
	if (iter == m_use_item_limit_cfg.end())
	{
...
const CrystalFbMonsterCfg::ItemCfg * RandActivityMiJingQiWenConfig2::GetCrystalFbMonsterCfg(int monster_group_id)const
{
...
const std::vector<int> RandActivityMiJingQiWenConfig2::GetMonsterDropId(int monster_npc_id, int play_times, int * village_id/* = NULL*/, int * prestige_num/* = NULL*/, int *add_exp/* = NULL*/, std::vector<ItemConfigData> * reward_list/* = NULL*/) const
{
	std::vector<int> ret;
...
const int RandActivityMiJingQiWenConfig2::GetRewardColumn(int play_times)const
{
...
const MiJingQiWenFbNPCRewardCfg2 * RandActivityMiJingQiWenConfig2::GetNpcRewardCfg(int npc_id, int option_index) const
{
	if (option_index < 0 || option_index >= 2)return NULL;
	std::map<int, MiJingQiWenFbNPCRewardListCfg2>::const_iterator it = m_npc_reward_cfg_map.find(npc_id);
...
const MiJingQiWenFbNPCRewardListCfg2 * RandActivityMiJingQiWenConfig2::GetNpcEventCfg(int npc_id)const
{
	std::map<int, MiJingQiWenFbNPCRewardListCfg2>::const_iterator it = m_npc_reward_cfg_map.find(npc_id);
	if (it != m_npc_reward_cfg_map.end())
	{
...
const int RandActivityMiJingQiWenConfig2::GetEventNpcCfg(int group_id, int rand_num, std::vector<new_Event_NPC_cfg2>* get_npc_list, std::vector<new_Event_NPC_cfg2>*has_create_npc_vec) const
{
	if (get_npc_list == NULL)return 0;
	std::map<int, std::vector<new_Event_NPC_cfg2> >::const_iterator it = m_event_npc_cfg_map.find(group_id);
	...
	{
		std::vector<new_Event_NPC_cfg2> node_vec;
		...
		if (has_create_npc_vec != NULL)
		{
			...
			std::vector<new_Event_NPC_cfg2>::iterator same_it2 = has_create_npc_vec->begin();//把需要取出的一个一个删除
			for (int size_vec = 0; size_vec < size_num && size_vec < 100; size_vec++)
			{
				for (std::vector<new_Event_NPC_cfg2>::iterator same_it = node_vec.begin(); same_it != node_vec.end(); same_it++)
				{
		...
		for (int i = 0; i < rand_num && i < 100; i++)
		{
			...
			int cum_count = 0;
			for (std::vector<new_Event_NPC_cfg2>::iterator itor = node_vec.begin(); itor != node_vec.end(); itor++)
			{
				cum_count += itor->rate;
...
const BuffListConfig * RandActivityMiJingQiWenConfig2::GetBuffId(int buff_group_id) const
{
...
const int RandActivityMiJingQiWenConfig2::RandNPCShopItemList(int npc_id, NpcShopItem but_list[MAX_ROLE_CRYSTALFB_BUY_ITEM_COUNT])const
{
...
const GatherNpcConfig * RandActivityMiJingQiWenConfig2::GetGatherNpcCfg(int npc_id) const
{
...
const int RandActivityMiJingQiWenConfig2::GetWorkshopItemWeightCfg(int group_id)const
{
...
const std::vector<WorkshopItemConfig> * RandActivityMiJingQiWenConfig2::GetWorkshopItemCfg(int group_id)const
{
...
const MiJingQiWenFbLevelTypeCfg2 * RandActivityMiJingQiWenConfig2::GetLevelToCfg(int role_level) const
{
	for (std::vector<MiJingQiWenFbLevelTypeCfg2>::const_iterator it = m_level_to_seq_vec.begin(); it != m_level_to_seq_vec.end(); it++)
	{
...
int RandActivityMiJingQiWenConfig2::InitLevelCfg(TiXmlElement * RootElement)
{
...
int RandActivityMiJingQiWenConfig2::InitBoxCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitRandNumCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitRewardCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitPosCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitSceneCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitUseLimitCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitMonsterCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitMonsterDropCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitPlayTimesToRewardColumnCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitEventGroupCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
	...
	while (NULL != root_element)
	{
		...
		if (!GetSubNodeValue(root_element, "npc_type", node_cfg.npc_type) ||
			node_cfg.npc_type < new_Event_NPC_cfg2::CRYSTALFB_NPC_TYPE_0 ||
			node_cfg.npc_type >  new_Event_NPC_cfg2::CRYSTALFB_NPC_TYPE_11)
		{
			return -4;
		}
...
int RandActivityMiJingQiWenConfig2::InitEventListCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
	...
	while (NULL != root_element)
	{
		...
		MiJingQiWenFbNPCRewardListCfg2 &node_cfg = m_npc_reward_cfg_map[npc_id];

		int operation_id = 0;
		if (!GetSubNodeValue(root_element, "operation_id", operation_id) || operation_id <= 0 || operation_id > MiJingQiWenFbNPCRewardListCfg2::MAX_OPERATION_INDEX)
		{
			return -2;
		}

		int reward_type = 0;
		if (!GetSubNodeValue(root_element, "reward_type", reward_type) || reward_type <= 0 || reward_type >= MiJingQiWenFbNPCRewardCfg2::REWARD_TYPE_MAX)
		{
			return -3;
		}
...
int RandActivityMiJingQiWenConfig2::InitShopListCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitBuffListCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitGatherNpcCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitWrokShopItemCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
...
int RandActivityMiJingQiWenConfig2::InitMiJingQiWenLevelTypeCfg(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element)
	{
	...
	while (NULL != root_element)
	{
		MiJingQiWenFbLevelTypeCfg2 node;
...
void RandActivityMiJingQiWenConfig2::RefreshPos(CrystalFbSceneCfg::ItemCfg ARG_OUT & cfg) const
{
