#ifndef __RA_MI_JING_QI_WEN_CONFIG_2_HPP__
#define __RA_MI_JING_QI_WEN_CONFIG_2_HPP__

#include "config/randactivityconfig/randactivityconfig.hpp"
#include "other/rolecrystalfb/rolecrystalfbconfig.hpp"

struct new_Event_NPC_cfg2
{
	...
	new_Event_NPC_cfg2() : event_group_id(0), npc_id(0), rate(0), npc_type(0) {}
	int event_group_id;
	int npc_id;
	int rate;
	int npc_type;
...
struct MiJingQiWenFbNPCRewardCfg2
{
	enum REWARD_TYPE
	{
	...
	MiJingQiWenFbNPCRewardCfg2() :npc_id(0), option_index(0), reward_type(0), is_delete(0)
	{
	}
...
struct MiJingQiWenFbNPCRewardListCfg2
{
	const static int MAX_OPERATION_INDEX = 2;//最大选择个数
	MiJingQiWenFbNPCRewardListCfg2()
	{
	}
	MiJingQiWenFbNPCRewardCfg2 reward_list[MAX_OPERATION_INDEX];
};

struct MiJingQiWenOtherCfg2
{
	MiJingQiWenOtherCfg2()
	{
		item_num = 0;
		random_event_num = 0;
		question_num = 0;
...
struct MiJingQiWenFbLevelTypeCfg2
{
	MiJingQiWenFbLevelTypeCfg2() : type(0), level_down(0), level_up(0) {}
	int type;
	int	level_down;
	int	level_up;
};
...
class RandActivityMiJingQiWenConfig2 : public RandActivityConfig
{
public:
	RandActivityMiJingQiWenConfig2();
	virtual ~RandActivityMiJingQiWenConfig2();
	...
	const MiJingQiWenOtherCfg2 & GetCrystalFbOtherCfg()const;
	...
	const MiJingQiWenFbNPCRewardCfg2 * GetNpcRewardCfg(int npc_id, int option_index)const ;
	const MiJingQiWenFbNPCRewardListCfg2 * GetNpcEventCfg(int npc_id) const;
	const int GetEventNpcCfg(int group_id, int rand_num, std::vector<new_Event_NPC_cfg2>*get_npc_list, std::vector<new_Event_NPC_cfg2>*has_create_npc_vec)const;
	...
	const MiJingQiWenFbLevelTypeCfg2 * GetLevelToCfg(int role_level) const;
private:
	...
	MiJingQiWenOtherCfg2 m_other_cfg;
	...
	std::map<int, std::vector<new_Event_NPC_cfg2> > m_event_npc_cfg_map;
	std::map<int, MiJingQiWenFbNPCRewardListCfg2> m_npc_reward_cfg_map;
	...
	std::vector<MiJingQiWenFbLevelTypeCfg2> m_level_to_seq_vec;
};
