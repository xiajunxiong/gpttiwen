...
#include "randactivityshanhaibaodaiconfig.hpp"

RandActivityShanHaiBaoDaiConfig::RandActivityShanHaiBaoDaiConfig()
{
}

RandActivityShanHaiBaoDaiConfig::~RandActivityShanHaiBaoDaiConfig()
{
	std::vector<RAShanHaiBaoDaiSectionCfg>().swap(m_section_cfg);
}

bool RandActivityShanHaiBaoDaiConfig::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;
...
const RAShanHaiBaoDaiItemCfg * RandActivityShanHaiBaoDaiConfig::GetRAShanHaiBaoDaiItemCfg(RandActivityManager * ramgr, int seq) const
{
	if (seq < 0 || seq >= RA_SHAN_HAI_BAO_DAI_MAX_ITEM) return NULL;

	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_SHAN_HAI_BAO_DAI);
	for (std::vector<RAShanHaiBaoDaiSectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityShanHaiBaoDaiConfig::GetRAShanHaiBaoDaiItemNum(RandActivityManager * ramgr) const
{
	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_SHAN_HAI_BAO_DAI);
	for (std::vector<RAShanHaiBaoDaiSectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityShanHaiBaoDaiConfig::InitBuyCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_section_cfg.size() > 0)
		{
				...
				section_end != m_section_cfg[pre_index].section_end)
			{
				...
				next_seq = 0;
				m_section_cfg.push_back(RAShanHaiBaoDaiSectionCfg());
			}
		}
		if (m_section_cfg.size() == 0)
		{
		...
		RAShanHaiBaoDaiSectionCfg & node_cfg = m_section_cfg[m_section_cfg.size() - 1];
		node_cfg.section_start = section_start;
		node_cfg.section_end = section_end;

		RAShanHaiBaoDaiItemCfg item_cfg;
		if (!GetSubNodeValue(data_element, "seq", item_cfg.seq) || item_cfg.seq != next_seq || item_cfg.seq >= RA_SHAN_HAI_BAO_DAI_MAX_ITEM) return -__LINE__;
...
int RandActivityShanHaiBaoDaiConfig::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
