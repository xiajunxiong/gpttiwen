...
#include "randactivityshanhaibaoxiangconfig.hpp"

RandActivityShanHaiBaoXiangConfig::RandActivityShanHaiBaoXiangConfig()
{
}

RandActivityShanHaiBaoXiangConfig::~RandActivityShanHaiBaoXiangConfig()
{
	std::vector<RAShanHaiBaoXiangSectionCfg>().swap(m_section_cfg);
}

bool RandActivityShanHaiBaoXiangConfig::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;
...
const RAShanHaiBaoXiangItemCfg * RandActivityShanHaiBaoXiangConfig::GetRAShanHaiBaoXiangItemCfg(RandActivityManager * ramgr, int seq) const
{
	if (seq < 0 || seq >= RA_SHAN_HAI_BAO_XIANG_MAX_ITEM) return NULL;

	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_SHAN_HAI_BAO_XIANG);
	for (std::vector<RAShanHaiBaoXiangSectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
const RAShanHaiBaoXiangRewardGroup * RandActivityShanHaiBaoXiangConfig::GetRAShanHaiBaoXiangRewardGroup(int reward_group_id) const
{
	std::map<int, RAShanHaiBaoXiangRewardGroup>::const_iterator iter = m_reward_group.find(reward_group_id);
...
int RandActivityShanHaiBaoXiangConfig::GetRAShanHaiBaoXiangItemNum(RandActivityManager * ramgr) const
{
	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_SHAN_HAI_BAO_XIANG);
	for (std::vector<RAShanHaiBaoXiangSectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityShanHaiBaoXiangConfig::InitBuyCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_section_cfg.size() > 0)
		{
				...
				section_end != m_section_cfg[pre_index].section_end)
			{
				...
				next_seq = 0;
				m_section_cfg.push_back(RAShanHaiBaoXiangSectionCfg());
			}
		}
		if (m_section_cfg.size() == 0)
		{
		...
		RAShanHaiBaoXiangSectionCfg & node_cfg = m_section_cfg[m_section_cfg.size() - 1];
		node_cfg.section_start = section_start;
		node_cfg.section_end = section_end;

		RAShanHaiBaoXiangItemCfg item_cfg;
		if (!GetSubNodeValue(data_element, "seq", item_cfg.seq) || item_cfg.seq != next_seq || item_cfg.seq >= RA_SHAN_HAI_BAO_XIANG_MAX_ITEM) return -__LINE__;
...
int RandActivityShanHaiBaoXiangConfig::InitRewardGroupCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		RAShanHaiBaoXiangRewardGroup & item_cfg = m_reward_group[group_id];
...
int RandActivityShanHaiBaoXiangConfig::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
