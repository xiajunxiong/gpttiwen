#include "randactivitysummerlilianconfig.hpp"

#include "item/itempool.h"
#include "monster/monster_group_pool.hpp"

RandActivitySummerLiLianConfig::RandActivitySummerLiLianConfig()
{
}

RandActivitySummerLiLianConfig::~RandActivitySummerLiLianConfig()
{
	std::vector<RASummerLiLianSectionCfg>().swap(m_base_cfg_vec);
	for (std::map<int, std::vector<RASummerLiLianBossGroupCfg> >::iterator it = m_boss_group_cfg.begin(); it != m_boss_group_cfg.end(); it++)
	{
		std::vector<RASummerLiLianBossGroupCfg>().swap(it->second);
	}
}

bool RandActivitySummerLiLianConfig::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;

	LOAD_RA_CONFIG("leader_configuration", InitBossgGroupCfg);
	LOAD_RA_CONFIG("reward_group", InitRewardGroupCfg);
	LOAD_RA_CONFIG("summer_lilian", InitBaseCfg);
	LOAD_RA_CONFIG("other", InitOtherCfg);		//需要放在奖励组读取之后

	return true;
}

const RASummerLiLianBaseCfg * RandActivitySummerLiLianConfig::GetBaseCfg(RandActivityManager * ramgr, int activity_time)const
{
	if(NULL == ramgr || activity_time <= 0 || activity_time > MAX_RA_SUMMER_LI_LIAN_BOSS_NUM) return NULL;

	int act_real_open_day = this->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_SUMMER_LI_LIAN);
	for (int i = 0; i < (int)m_base_cfg_vec.size(); i++)
	{
		...
		{
			std::map<int, RASummerLiLianBaseCfg>::const_iterator it = m_base_cfg_vec[i].m_base_cfg.find(activity_time);
...
const std::map<int, RASummerLiLianBaseCfg> * RandActivitySummerLiLianConfig::GetBaseMapCfg(RandActivityManager * ramgr) const
{
...
const RASummerLiLianBossGroupCfg * RandActivitySummerLiLianConfig::GetBossGroupCfg(int boss_group_id, int role_level)const
{
	if(role_level <= 0 || role_level > MAX_ROLE_LEVEL) return NULL;

	std::map<int, std::vector<RASummerLiLianBossGroupCfg> >::const_iterator it = m_boss_group_cfg.find(boss_group_id);
...
const RASummerLiLianRewardCfg * RandActivitySummerLiLianConfig::GetRewardCfg(int reward_group_id)const
{
	std::map<int, RASummerLiLianRewardGroupCfg>::const_iterator it = m_reward_group_cfg.find(reward_group_id);
...
int RandActivitySummerLiLianConfig::InitOtherCfg(TiXmlElement * RootElement)
{
...
int RandActivitySummerLiLianConfig::InitBossgGroupCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		RASummerLiLianBossGroupCfg node_cfg;
...
int RandActivitySummerLiLianConfig::InitRewardGroupCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		RASummerLiLianRewardCfg node_cfg;
...
int RandActivitySummerLiLianConfig::InitBaseCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
	...
	while (NULL != data_element)
	{
		...
		if (m_base_cfg_vec.size() > 0)
		{
				...
				section_end != m_base_cfg_vec[pre_index].section_end)
			{
				...
				m_base_cfg_vec.push_back(RASummerLiLianSectionCfg());
			}
		}
		if (m_base_cfg_vec.size() == 0)
		{
		...
		RASummerLiLianSectionCfg & node_cfg = m_base_cfg_vec[m_base_cfg_vec.size() - 1];
		...
		RASummerLiLianBaseCfg reward_cfg;
