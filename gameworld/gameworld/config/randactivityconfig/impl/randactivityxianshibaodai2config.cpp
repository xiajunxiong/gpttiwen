...
#include "randactivityxianshibaodai2config.hpp"

RandActivityXianShiBaoDai2Config::RandActivityXianShiBaoDai2Config()
{
}

RandActivityXianShiBaoDai2Config::~RandActivityXianShiBaoDai2Config()
{
	std::vector<RAXianShiBaoDai2SectionCfg>().swap(m_section_cfg);
}

bool RandActivityXianShiBaoDai2Config::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;
...
const RAXianShiBaoDai2ItemCfg * RandActivityXianShiBaoDai2Config::GetRAXianShiBaoDaiItemCfg(RandActivityManager * ramgr, int seq) const
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_DAI_MAX_ITEM) return NULL;

	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_DAI_2);
	for (std::vector<RAXianShiBaoDai2SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityXianShiBaoDai2Config::GetRAXianShiBaoDaiItemNum(RandActivityManager * ramgr) const
{
	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_DAI_2);
	for (std::vector<RAXianShiBaoDai2SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityXianShiBaoDai2Config::InitBuyCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_section_cfg.size() > 0)
		{
			    ...
			    section_end != m_section_cfg[pre_index].section_end)
			{
				...
				next_seq = 0;
				m_section_cfg.push_back(RAXianShiBaoDai2SectionCfg());
			}
		}
		if (m_section_cfg.size() == 0)
		{
		...
		RAXianShiBaoDai2SectionCfg & node_cfg = m_section_cfg[m_section_cfg.size() - 1];
		node_cfg.section_start = section_start;
		node_cfg.section_end = section_end;

		RAXianShiBaoDai2ItemCfg item_cfg;
...
int RandActivityXianShiBaoDai2Config::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
