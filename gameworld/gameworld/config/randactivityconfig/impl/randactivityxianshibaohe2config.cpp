...
#include "randactivityxianshibaohe2config.hpp"

RandActivityXianShiBaoHe2Config::RandActivityXianShiBaoHe2Config()
{
}

RandActivityXianShiBaoHe2Config::~RandActivityXianShiBaoHe2Config()
{
	std::vector<RAXianShiBaoHe2SectionCfg>().swap(m_section_cfg);
}

bool RandActivityXianShiBaoHe2Config::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;
...
const RAXianShiBaoHe2ItemCfg * RandActivityXianShiBaoHe2Config::GetRAXianShiBaoHeItemCfg(RandActivityManager * ramgr, int seq) const
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_HE_2_MAX_ITEM) return NULL;

	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_HE_2);
	for (std::vector<RAXianShiBaoHe2SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityXianShiBaoHe2Config::GetRAXianShiBaoHeItemNum(RandActivityManager * ramgr) const
{
	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_HE_2);
	for (std::vector<RAXianShiBaoHe2SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityXianShiBaoHe2Config::InitBuyCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_section_cfg.size() > 0)
		{
			    ...
			    section_end != m_section_cfg[pre_index].section_end)
			{
				...
				next_seq = 0;
				m_section_cfg.push_back(RAXianShiBaoHe2SectionCfg());
			}
		}
		if (m_section_cfg.size() == 0)
		{
		...
		RAXianShiBaoHe2SectionCfg & node_cfg = m_section_cfg[m_section_cfg.size() - 1];
		node_cfg.section_start = section_start;
		node_cfg.section_end = section_end;

		RAXianShiBaoHe2ItemCfg item_cfg;
		if (!GetSubNodeValue(data_element, "seq", item_cfg.seq) || item_cfg.seq != next_seq || item_cfg.seq >= RA_XIAN_SHI_BAO_HE_2_MAX_ITEM) return -__LINE__;
...
int RandActivityXianShiBaoHe2Config::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
