...
#include "randactivityxianshibaohe4config.hpp"

RandActivityXianShiBaoHe4Config::RandActivityXianShiBaoHe4Config()
{
}

RandActivityXianShiBaoHe4Config::~RandActivityXianShiBaoHe4Config()
{
	std::vector<RAXianShiBaoHe4SectionCfg>().swap(m_section_cfg);
}

bool RandActivityXianShiBaoHe4Config::Init(TiXmlElement * RootElement, std::string * err)
{
	PRE_LOAD_RA_CONFIG;
...
const RAXianShiBaoHe4ItemCfg * RandActivityXianShiBaoHe4Config::GetRAXianShiBaoHeItemCfg(RandActivityManager * ramgr, int seq) const
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_HE_4_MAX_ITEM) return NULL;

	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_HE_4);
	for (std::vector<RAXianShiBaoHe4SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityXianShiBaoHe4Config::GetRAXianShiBaoHeItemNum(RandActivityManager * ramgr) const
{
	int act_real_open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActRealOpenDay(ramgr, RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_HE_4);
	for (std::vector<RAXianShiBaoHe4SectionCfg>::const_iterator it = m_section_cfg.begin(); it != m_section_cfg.end(); it++)
	{
...
int RandActivityXianShiBaoHe4Config::GetConfigVer() const
{
	return m_other_cfg.cfg_ver;
}

int RandActivityXianShiBaoHe4Config::InitBuyCfg(TiXmlElement * RootElement)
{
	...
	while (NULL != data_element)
	{
		...
		if (m_section_cfg.size() > 0)
		{
			    ...
			    section_end != m_section_cfg[pre_index].section_end)
			{
				...
				next_seq = 0;
				m_section_cfg.push_back(RAXianShiBaoHe4SectionCfg());
			}
		}
		if (m_section_cfg.size() == 0)
		{
		...
		RAXianShiBaoHe4SectionCfg & node_cfg = m_section_cfg[m_section_cfg.size() - 1];
		node_cfg.section_start = section_start;
		node_cfg.section_end = section_end;

		RAXianShiBaoHe4ItemCfg item_cfg;
		if (!GetSubNodeValue(data_element, "seq", item_cfg.seq) || item_cfg.seq != next_seq || item_cfg.seq >= RA_XIAN_SHI_BAO_HE_4_MAX_ITEM) return -__LINE__;
...
int RandActivityXianShiBaoHe4Config::InitOtherCfg(TiXmlElement * RootElement)
{
	TiXmlElement * data_element = RootElement->FirstChildElement("data");
	if (NULL == data_element)
	{
