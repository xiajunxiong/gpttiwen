#ifndef __ACTIVITY_WORLD_BOSS_HPP__
#define __ACTIVITY_WORLD_BOSS_HPP__
...
{
	WorldBossRoleInfo() : uid(0), hurt(0), heal(0), play_times(0)
	{
	}
...
class ActivityWorldBoss : public Activity
{
public:
	ActivityWorldBoss(ActivityManager *activity_manager);
	~ActivityWorldBoss();
	...
	void SendReward();
	void HiddenGameSendRankResult(crossgameprotocal::HiddenGameWorldBossSyncRankResultMsg* msg);
	void HiddenGameSyncUidList(crossgameprotocal::HiddenGameWorldBossSyncParticipateUidList* msg);
	...
	void SetInfo(crossgameprotocal::CrossGameSyncWorldBossInfo* msg);
...
private:
	...
	std::vector<WorldBossRoleInfo> GetLucklist();
	void UpdateRank();
	...
	std::map<int, WorldBossRoleInfo> m_role_info;
	...
	std::vector<WorldBossRoleInfo> m_hurt_rank;
	std::vector<WorldBossRoleInfo> m_heal_rank;
