#ifndef __ACTIVITY_WORLD_BOSS2_HPP__
#define __ACTIVITY_WORLD_BOSS2_HPP__
...
struct WorldBossRoleInfo2
{
	WorldBossRoleInfo2() : uid(0), hurt(0), heal(0), play_times(0)
	{
	}
...
class ActivityWorldBoss2 : public Activity
{
public:
	ActivityWorldBoss2(ActivityManager *activity_manager);
	~ActivityWorldBoss2();
	...
	void SendReward();
	void HiddenGameSendRankResult(crossgameprotocal::HiddenGameWorldBoss2SyncRankResultMsg* msg);
	void HiddenGameSyncUidList(crossgameprotocal::HiddenGameWorldBoss2SyncParticipateUidList* msg);
	...
	int GetWorldBoss2PlayTimes(int uid);
	void AddWorldBoss2PlayTimes(int uid,int play_times = 1);

	void SyncInfoToLocalServer(bool is_send_boss_pos = false);
	void SetInfo(crossgameprotocal::CrossGameSyncWorldBoss2Info* msg);
...
private:
	...
	std::vector<WorldBossRoleInfo2> GetLucklist();
	void UpdateRank();
	...
	std::map<int, WorldBossRoleInfo2> m_role_info;
	...
	std::vector<WorldBossRoleInfo2> m_hurt_rank;
	std::vector<WorldBossRoleInfo2> m_heal_rank;
