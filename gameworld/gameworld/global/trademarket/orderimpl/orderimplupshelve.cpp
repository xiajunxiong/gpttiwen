...
OrderImplUpshelve::OrderImplUpshelve(int relate_role_id) : OrderBase(TRADEMARKET_ORDER_TYPE_UPSHELVE, relate_role_id)
{

}

void OrderImplUpshelve::OnOrderFailed(bool is_return_resource)
{
	gamelog::g_log_trade_market.printf(LL_WARNING, "OrderImplUpshelve::OnOrderFailed role_id[%d] \
	...
	if (is_return_resource && m_order_status >= TRADEMARKET_ORDER_STATUS_CONSUME) // 消耗过的才能返还
	{
		...
		SNPRINTF(contentparam.contenttxt, sizeof(contentparam.contenttxt), gamestring::g_order_upshelve_return_resource);
...
void OrderImplUpshelve::OnOrderFinish()
{
	gamelog::g_log_trade_market.printf(LL_INFO, "OrderImplUpshelve::OnOrderFinish role_id[%d] order_id[%lld]",
