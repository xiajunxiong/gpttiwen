#include "maintaskcatchfoxconfig.hpp"
#include "world.h"
#include "gameworld/task/taskpool.h"
#include "item/itempool.h"

MainTaskCatchFoxFbConfig::MainTaskCatchFoxFbConfig()
{
}

MainTaskCatchFoxFbConfig::~MainTaskCatchFoxFbConfig()
{
	std::vector<MainTaskCatchFoxMonsterConfig>().swap(m_monster_cfg_vec);
}

bool MainTaskCatchFoxFbConfig::Init(const std::string & configname, std::string * err)
{
	PRE_LOAD_CONFIG
...
const MainTaskCatchFoxMonsterConfig * MainTaskCatchFoxFbConfig::GetMonsterConfig(int level)
{
	for (std::vector<MainTaskCatchFoxMonsterConfig>::iterator it = m_monster_cfg_vec.begin(); it != m_monster_cfg_vec.end(); it++)
	{
...
int MainTaskCatchFoxFbConfig::InitOhterCfg(TiXmlElement * RootElement)
{
...
int MainTaskCatchFoxFbConfig::InitFindFoxConfig(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element) return -1000;
...
int MainTaskCatchFoxFbConfig::InitLevelMonsterGroup(TiXmlElement * RootElement)
{
	TiXmlElement * root_element = RootElement->FirstChildElement("data");
	if (NULL == root_element) return -1000;
	...
	{
		MainTaskCatchFoxMonsterConfig node;
