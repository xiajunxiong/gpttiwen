...
#include "roleactivitydingzhibaoxiang.hpp"


RoleActivityDingZhiBaoXiang::RoleActivityDingZhiBaoXiang(int type) :RoleActivity(type)
{
}

RoleActivityDingZhiBaoXiang::~RoleActivityDingZhiBaoXiang()
{
}

void RoleActivityDingZhiBaoXiang::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityDingZhiBaoXiang::GetInitParam(RoleActivityRoleData * p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityDingZhiBaoXiang::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityDingZhiBaoXiang::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
...
void RoleActivityDingZhiBaoXiang::CheckInitRandActivity(int rand_act_type, bool is_force)
{
...
bool RoleActivityDingZhiBaoXiang::IsFinishBonusCond(int type)
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	const RandActivityDingZhiBaoXiangConfig * config = static_cast<const RandActivityDingZhiBaoXiangConfig *>
	...
	bool is_finish = true;
	for (int i = 0; i < item_num && i < RA_DING_ZHI_BAO_XIANG_BUY_MAX_ITEM; i++)
	{
...
void RoleActivityDingZhiBaoXiang::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
	switch (raor->opera_type)
	{
		...
		case 2:
			{
				this->ReceiveFreeReward();				// 定制宝箱免费道具
			}
			break;
	}
}

bool RoleActivityDingZhiBaoXiang::OnBuyItemOld(int seq, int param1, int chong_zhi_gold)
{
	if (seq < 0 || seq >= RA_DING_ZHI_BAO_XIANG_MAX_ITEM)
	{
		return false;
	}
	...
	const RADingZhiBaoXiangItemCfg * buy_cfg = config->GetRADingZhiBaoXiangItemCfg(ramgr, seq);
	if (buy_cfg == NULL)
	{
		return false;
	}
	...
	const RADingZhiBaoXiangRewardGroup * reward_group_cfg = config->GetRADingZhiBaoXiangRewardGroup(buy_cfg->reward_group_id);
...
int RoleActivityDingZhiBaoXiang::OnBuyGiftItem(int seq, ARG_OUT int * price)
{
	if (seq < 0 || seq >= RA_DING_ZHI_BAO_XIANG_MAX_ITEM)
	{
		return 1;
...
int RoleActivityDingZhiBaoXiang::OnBuyItemCheck(int seq, int param1, long long ARG_OUT & unique_key)
{
	unique_key = 0;
	...
	CmdBuyRecordManager::Instance().DelRedundantCmdBuyRecord< RoleCommonSaveCmdBuyItem >(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, gamelog::g_log_ding_zhi_bao_xiang, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();

	std::string log_str2 = ::GetString(*buy_item);
	gamelog::g_log_ding_zhi_bao_xiang.printf(LL_INFO, "%s line:%d |  role[%d %s] %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), log_str2.c_str());

	return 0;
}

int RoleActivityDingZhiBaoXiang::OnBuyItem(int chong_zhi_gold, long long unique_id)
{
	...
	gamelog::g_log_ding_zhi_bao_xiang.printf(LL_INFO, "%s line:%d | role[%d %s] ra_begin_timestamp:%u %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(),
	...
	{
		if (0 <= seq && seq < RA_DING_ZHI_BAO_XIANG_MAX_ITEM)
		{
			if (limit_type == 1)
			{
				m_param.ra_ding_zhi_bao_xiang_buy_times[seq] += 1;
	...
	CmdBuyRecordManager::Instance().DelCmdBuyRecordByUniqueId<RoleCommonSaveCmdBuyItem>(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, unique_id, gamelog::g_log_ding_zhi_bao_xiang, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();
...
void RoleActivityDingZhiBaoXiang::SendInfo()
{
	Protocol::SCRADingZhiBaoXiangInfo protocol;
...
void RoleActivityDingZhiBaoXiang::ReceiveFreeReward()
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const RADingZhiBaoXiangOtherCfg & other_cfg = config->GetOtherCfg();
