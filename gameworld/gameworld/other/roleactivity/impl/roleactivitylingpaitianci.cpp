...
#include "roleactivitylingpaitianci.hpp"

RoleActivityLingPaiTianCi::RoleActivityLingPaiTianCi(int type) : RoleActivity(type)
{
}

RoleActivityLingPaiTianCi::~RoleActivityLingPaiTianCi()
{
}

void RoleActivityLingPaiTianCi::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityLingPaiTianCi::GetInitParam(RoleActivityRoleData * p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityLingPaiTianCi::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityLingPaiTianCi::CheckInitRandActivity(int rand_act_type, bool is_force)
{
...
void RoleActivityLingPaiTianCi::OnAddChongZhi(long long chongzhi)
{
...
void RoleActivityLingPaiTianCi::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
...
int RoleActivityLingPaiTianCi::FetchRewards(int seq)
{
	if (seq < 0 || seq >= RA_LING_PAI_TIAN_CI_MAX_ITEM)
	{
		return -__LINE__;
	}
	...
	const RandActivityLingPaiTianCiConfig * config = static_cast<const RandActivityLingPaiTianCiConfig *>
	...
	const RALingPaiTianCiCfg * cfg = config->GetRALingPaiTianCiCfgCfg(ramgr, seq);
	if (cfg == NULL)
	{
		return -__LINE__;
	}
	...
	{
		// 传闻
		int length = SNPRINTF(gamestring::GAMESTRING_BUF, sizeof(gamestring::GAMESTRING_BUF), gamestring::g_ling_pai_tian_ci_borcast,
...
void RoleActivityLingPaiTianCi::SendInfo()
{
	Protocol::SCRALingPaiTianCiInfo protocol;
