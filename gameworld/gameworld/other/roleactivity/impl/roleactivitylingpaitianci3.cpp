...
#include "roleactivitylingpaitianci3.hpp"

RoleActivityLingPaiTianCi3::RoleActivityLingPaiTianCi3(int type) : RoleActivity(type)
{
}

RoleActivityLingPaiTianCi3::~RoleActivityLingPaiTianCi3()
{
}

void RoleActivityLingPaiTianCi3::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityLingPaiTianCi3::GetInitParam(RoleActivityRoleData * p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityLingPaiTianCi3::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityLingPaiTianCi3::CheckInitRandActivity(int rand_act_type, bool is_force)
{
...
void RoleActivityLingPaiTianCi3::OnAddChongZhi(long long chongzhi)
{
...
void RoleActivityLingPaiTianCi3::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
...
int RoleActivityLingPaiTianCi3::FetchRewards(int seq)
{
	if (seq < 0 || seq >= RA_LING_PAI_TIAN_CI_3_MAX_ITEM)
	{
		return -__LINE__;
	}
	...
	const RandActivityLingPaiTianCi3Config * config = static_cast<const RandActivityLingPaiTianCi3Config *>
	...
	const RALingPaiTianCi3Cfg * cfg = config->GetRALingPaiTianCi3CfgCfg(ramgr, seq);
	if (cfg == NULL)
	{
		return -__LINE__;
	}
	...
	{
		// 传闻
		int length = SNPRINTF(gamestring::GAMESTRING_BUF, sizeof(gamestring::GAMESTRING_BUF), gamestring::g_fu_shi_hun_qi_borcast,
...
void RoleActivityLingPaiTianCi3::SendInfo()
{
	Protocol::SCRALingPaiTianCi3Info protocol;
