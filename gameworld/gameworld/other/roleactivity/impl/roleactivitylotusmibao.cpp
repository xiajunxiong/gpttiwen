#include "roleactivitylotusmibao.hpp"
#include "gamelog.h"
...
RoleActivityLotusMiBao::RoleActivityLotusMiBao(int type)
	:RoleActivity(type)
{
}

RoleActivityLotusMiBao::~RoleActivityLotusMiBao()
{
}

void RoleActivityLotusMiBao::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(RALotusMiBao));
}

void RoleActivityLotusMiBao::GetInitParam(RoleActivityRoleData * p)
{
	memcpy(p->base_data, &m_param, sizeof(RALotusMiBao));
}

void RoleActivityLotusMiBao::OnLoginSendInfo()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;

	if (!ramgr->IsRandActivityOpen(RAND_ACTIVITY_TYPE_LOTUS_MI_BAO))
	{
		return;
	}
	this->CheckInitRandActivity(RAND_ACTIVITY_TYPE_LOTUS_MI_BAO);
	this->SendInfo();
}

void RoleActivityLotusMiBao::CheckInitRandActivity(int rand_act_type, bool is_force)
{
	if (RAND_ACTIVITY_TYPE_LOTUS_MI_BAO != rand_act_type)
	{
		return;
	}
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
...
void RoleActivityLotusMiBao::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
	if (NULL == raor)
	{
		return;
	...
	{
		case Protocol::RA_LOTUS_MI_BAO_REQ_TYPE_INFO:
		{
			this->SendInfo();
		}
		break;
		case Protocol::RA_LOTUS_MI_BAO_REQ_TYPE_ACTIVE_GRID:
		{	
			this->OnActiveGrid(raor->param_1, raor->param_2);
		}
		break;
		case Protocol::RA_LOTUS_MI_BAO_REQ_TYPE_FETCH_ACTIVE_ALL_REWARD:
		{
...
void RoleActivityLotusMiBao::SendInfo()
{
	Protocol::SCRALotusMiBaoInfo info;
	info.reserve_sh = 0;
	info.reserve_ch = 0;
...
void RoleActivityLotusMiBao::OnActiveGrid(int row, int column)
{
	...
	const RandActivityLotusMiBaoConfig * config = static_cast<const RandActivityLotusMiBaoConfig*>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(RAND_ACTIVITY_TYPE_LOTUS_MI_BAO));
	if (NULL == config)
	{
		m_role_module_mgr->NoticeNum(errornum::EN_UNKNOWN_ERROR);
		return;
	}
	const RALotusMiBaoCfg * base_cfg = config->GetBaseCfg(ramgr, row, column);
	if (NULL == base_cfg)
	{
		return;
	}
	...
	gamelog::g_log_cool_summer.printf(LL_INFO, "%s user[%d, %s], level[%d], activity_type[%d], row[%d], column[%d]", __FUNCTION__,
		...
		RAND_ACTIVITY_TYPE_LOTUS_MI_BAO, row, column);
}

void RoleActivityLotusMiBao::OnFetchAllActiveReward()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	const RALotsMiBaoFinishCondRewardCfg * cond_reward_cfg = config->GetCondRewardCfg(RA_LOTUS_MI_BAO_CONDITION_TYPE_ALL, MAX_RA_LOTUS_MI_BAO_GRID_NUM);
	if (NULL == cond_reward_cfg)
	{
		return;
	}
	std::vector<ItemConfigData> reward_list;
	const RALotusMiBaoRewardCfg * reward_cfg = config->GetRandRewardCfg(cond_reward_cfg->reward_group_id);
	if (NULL != reward_cfg)
	{
	...
	if (!reward_list.empty())
	{
		...
		{
			if (m_role_module_mgr->GetKnapsack()->PutList((short)reward_list.size(), &reward_list[0], PUT_REASON_LOTUS_MI_BAO))
			{
				is_put_succ = true;
			}
		}
		if (!is_put_succ)
		{
			m_role_module_mgr->GetKnapsack()->SendMail(&reward_list[0], (short)reward_list.size(), SEND_MAIL_TYPE_ACTIVITY_REWARD, true, RAND_ACTIVITY_TYPE_LOTUS_MI_BAO);
		}
		m_role_module_mgr->GetKnapsack()->NotifyGetItemList((short)reward_list.size(), &reward_list[0], GET_REASON_ACTVITIY_REWARD, RAND_ACTIVITY_TYPE_LOTUS_MI_BAO);
	}
	...
	gamelog::g_log_cool_summer.printf(LL_INFO, "%s user[%d, %s], level[%d], activity_type[%d]", __FUNCTION__,
		m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), m_role_module_mgr->GetRole()->GetLevel(),
		RAND_ACTIVITY_TYPE_LOTUS_MI_BAO);
	...
	{
		shan_hai_meet->OnFinishType(RA_SHAN_HAI_MEET_ACT_TYPE_5, 1);
	}
}

void RoleActivityLotusMiBao::OnCheckCondFinish(int row, int column)
{
	if (row < 0 || row >= MAX_RA_LOTUS_MI_BAO_ROW_NUM || column < 0 || column >= MAX_RA_LOTUS_MI_BAO_COLUMN_NUM) return;
	...
	const RALotsMiBaoFinishCondRewardCfg * row_cond_reward_cfg = config->GetCondRewardCfg(RA_LOTUS_MI_BAO_CONDITION_TYPE_ROW, row + 1);
	if (NULL != row_cond_reward_cfg)
	{
		...
		{
			const RALotusMiBaoRewardCfg * reward_cfg = config->GetRandRewardCfg(row_cond_reward_cfg->reward_group_id);
			std::vector<ItemConfigData> reward_list;
			...
			if (NULL != shan_hai_meet)
			{
				shan_hai_meet->OnFinishType(RA_SHAN_HAI_MEET_ACT_TYPE_0, 1);
			}
			...
			gamelog::g_log_cool_summer.printf(LL_INFO, "%s user[%d, %s], level[%d], activity_type[%d] cond[%d] row[%d]", __FUNCTION__,
				m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), m_role_module_mgr->GetRole()->GetLevel(),
				RAND_ACTIVITY_TYPE_LOTUS_MI_BAO, RA_LOTUS_MI_BAO_CONDITION_TYPE_ROW, row);
		}
	}
	const RALotsMiBaoFinishCondRewardCfg * column_cond_reward_cfg = config->GetCondRewardCfg(RA_LOTUS_MI_BAO_CONDITION_TYPE_COLUMN, column + 1);
	if (NULL != column_cond_reward_cfg)
	{
		...
		{
			const RALotusMiBaoRewardCfg * reward_cfg = config->GetRandRewardCfg(column_cond_reward_cfg->reward_group_id);
			std::vector<ItemConfigData> reward_list;
			if (NULL != reward_cfg && !reward_cfg->reward_list.empty())
			{
				reward_list.assign(reward_cfg->reward_list.begin(), reward_cfg->reward_list.end());
			...
			gamelog::g_log_cool_summer.printf(LL_INFO, "%s user[%d, %s], level[%d], activity_type[%d] cond[%d] column[%d]", __FUNCTION__,
				m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), m_role_module_mgr->GetRole()->GetLevel(),
				RAND_ACTIVITY_TYPE_LOTUS_MI_BAO, RA_LOTUS_MI_BAO_CONDITION_TYPE_COLUMN, column);
		}
	}
	if (!notice_list.empty())
	{
		m_role_module_mgr->GetKnapsack()->NotifyGetItemList((short)notice_list.size(), &notice_list[0], GET_REASON_ACTVITIY_REWARD, RAND_ACTIVITY_TYPE_LOTUS_MI_BAO);
	}
}

bool RoleActivityLotusMiBao::IsFinishCondActive(int cond_type, int param1)
{
	switch (cond_type)
	{
