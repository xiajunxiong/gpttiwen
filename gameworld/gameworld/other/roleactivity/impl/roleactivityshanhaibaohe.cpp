#include "roleactivityshanhaibaohe.hpp"
...
RoleActivityShanHaiBaoHe::RoleActivityShanHaiBaoHe(int type)
	:RoleActivity(type)
{

}

RoleActivityShanHaiBaoHe::~RoleActivityShanHaiBaoHe()
{

}

void RoleActivityShanHaiBaoHe::Init(RoleModuleManager *mgr, const RoleActivityRoleData &p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityShanHaiBaoHe::GetInitParam(RoleActivityRoleData *p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityShanHaiBaoHe::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityShanHaiBaoHe::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
...
void RoleActivityShanHaiBaoHe::CheckInitRandActivity(int rand_act_type, bool is_force /*= false*/)
{
...
bool RoleActivityShanHaiBaoHe::IsFinishBonusCond(int type /*= 0*/)
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	const RandActivityShanHaiBaoHeConfig * config = static_cast<const RandActivityShanHaiBaoHeConfig *>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(m_role_activity_type));
	if (NULL == config)
	{
		return false;
	}
	int item_num = config->GetRAShanHaiBaoHeItemNum(ramgr);
	bool is_finish = true;
	for (int i = 0; i < item_num && i < RA_SHAN_HAI_BAO_HE_BUY_MAX_ITEM; i++)
	{
...
void RoleActivityShanHaiBaoHe::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq *raor)
{
...
int RoleActivityShanHaiBaoHe::OnBuyGiftItem(int seq, ARG_OUT int * price)
{
	if (seq < 0 || seq >= RA_SHAN_HAI_BAO_HE_MAX_ITEM)
	{
		return 1;
	}
	...
	if (!ramgr->IsRandActivityOpen(RAND_ACTIVITY_TYPE_SHAN_HAI_BAO_HE))
	{
		return 1;
	}

	const RAShanHaiBaoHeItemCfg * buy_cfg = config->GetRAShanHaiBaoHeItemCfg(ramgr, seq);
	if (buy_cfg == NULL)
	{
		return 1;
	}
	...
	{
		m_role_module_mgr->GetKnapsack()->PutList((short)buy_cfg->rewards.size(), &buy_cfg->rewards[0], PUT_REASON_SHAN_HAI_BAO_HE);
	}
	else
	{
		// 发邮件
...
int RoleActivityShanHaiBaoHe::OnBuyItemCheck(int seq, long long ARG_OUT & unique_key)
{
	unique_key = 0;
...
int RoleActivityShanHaiBaoHe::OnBuyItem(int chong_zhi_gold, long long unique_id)
{
	...
	{
		m_role_module_mgr->GetKnapsack()->PutList((short)rewards.size(), &rewards[0], PUT_REASON_SHAN_HAI_BAO_HE);
	}
	else
	{
		// 发邮件
	...
	{
		if (0 <= seq && seq < RA_SHAN_HAI_BAO_HE_MAX_ITEM)
		{
			if (limit_type == 1)
			{
				m_param.ra_buy_times[seq] += 1;
...
void RoleActivityShanHaiBaoHe::SendInfo()
{
	Protocol::SCRAShanHaiBaoHeInfo protocol;
...
void RoleActivityShanHaiBaoHe::ReceiveFreeReward()
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const RAShanHaiBaoHeOtherCfg & other_cfg = config->GetOtherCfg();
	...
	if (!m_role_module_mgr->GetKnapsack()->PutVec(other_cfg.free_reward, PUT_REASON_SHAN_HAI_BAO_HE))
	{
		return;
	}
