#include "roleactivityshanhaihuadeng.hpp"
#include "gamelog.h"
...
RoleActivityShanHaiHuaDeng::RoleActivityShanHaiHuaDeng(int type)
	:RoleActivity(type), next_refresh_light_timestamp(0)
{
	memset(m_light_list, 0, sizeof(m_light_list));
}

RoleActivityShanHaiHuaDeng::~RoleActivityShanHaiHuaDeng()
{
}

void RoleActivityShanHaiHuaDeng::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(RAShanHaiHuaDengParam));
}

void RoleActivityShanHaiHuaDeng::GetInitParam(RoleActivityRoleData * p)
{
	memcpy(p->base_data, &m_param, sizeof(RAShanHaiHuaDengParam));
}

void RoleActivityShanHaiHuaDeng::Update(time_t now_second)
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;

	if (!ramgr->IsRandActivityOpen(RAND_ACTIVITY_TYPE_SHANHAI_HUADENG))
	{
		return;
	}
	this->CheckInitRandActivity(RAND_ACTIVITY_TYPE_SHANHAI_HUADENG);
	if (0 != m_param.game_end_timestamp && (unsigned int)now_second >= m_param.game_end_timestamp)
	{
		...
		this->SendGameInfo(Protocol::RA_SHANHAI_HUADENG_GAME_NOTICE_TYPE_GAME_OVER);
		this->SendAllInfo();
		...
		gamelog::g_log_burning_summer.printf(LL_INFO, "%s user[%d, %s], level[%d], activity_type[%d], join_times[%d, %d] single_score[%d, %d], score_count[%d, %d]", __FUNCTION__,
			m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), m_role_module_mgr->GetRole()->GetLevel(), RAND_ACTIVITY_TYPE_SHANHAI_HUADENG,
...
void RoleActivityShanHaiHuaDeng::OnLoginSendInfo()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const RandActivityShanHaiHuaDengConfig * config = static_cast<const RandActivityShanHaiHuaDengConfig *>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(RAND_ACTIVITY_TYPE_SHANHAI_HUADENG));
	...
	if (0 != m_param.game_end_timestamp)
	{
		...
		{
			this->SendGameInfo(Protocol::RA_SHANHAI_HUADENG_GAME_NOTICE_TYPE_IN_THE_GAME);
...
void RoleActivityShanHaiHuaDeng::CheckInitRandActivity(int rand_act_type, bool is_force)
{
	if (RAND_ACTIVITY_TYPE_SHANHAI_HUADENG != rand_act_type)
	{
		return;
	}
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
...
void RoleActivityShanHaiHuaDeng::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
	...
	std::map<int, RAShanHaiHuaDengBaseCfg> task_list;
	config->GetTaskList(ramgr, task_list);
	if (!task_list.empty())
	{
		for (std::map<int, RAShanHaiHuaDengBaseCfg>::iterator it = task_list.begin(); it != task_list.end(); it++)
		{
			if (SHANHAI_HUADENG_TASK_TYPE_DAY == it->second.task_type)
			{
...
void RoleActivityShanHaiHuaDeng::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
	if (NULL == raor)
	{
		return;
	...
	this->CheckInitRandActivity(RAND_ACTIVITY_TYPE_SHANHAI_HUADENG);
	UNSTD_STATIC_CHECK(RA_SHANHAI_HUADENG_REQ_TYPE_MAX == 5);
	switch (raor->opera_type)
	{
	case RA_SHANHAI_HUADENG_REQ_TYPE_START_GAME:
		{
			this->OnStartGame();
		}
		break;
	case RA_SHANHAI_HUADENG_REQ_TYPE_CATCH_LANTERN:
		{
			this->OnCatchLantern(raor->param_1);
		}
		break;
	case RA_SHANHAI_HUADENG_REQ_TYPE_FETCH_REWARD:
		{
			this->OnFetchTaskReward(raor->param_1);
		}
		break;
	case RA_SHANHAI_HUADENG_REQ_TYPE_FORCE_CLOSE:
		{
			this->OnForceClose();
		}
		break;
	case RA_SHANHAI_HUADENG_REQ_TYPE_FLOW_AWAY:
		{
...
void RoleActivityShanHaiHuaDeng::OnStartGame()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	this->OnRefreshLight(now_second);
	this->SendGameInfo(Protocol::RA_SHANHAI_HUADENG_GAME_NOTICE_TYPE_START_GAME);

	this->OnParticipateRA(__FUNCTION__);
}

void RoleActivityShanHaiHuaDeng::OnCatchLantern(int type)
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	if (type < 0 || type >= MAX_RA_SHANHAI_HUADENG_LIGHT_TYPE_NUM)
	{
		return;
	}
	//判断花灯数量
...
void RoleActivityShanHaiHuaDeng::OnFetchTaskReward(int task_id)
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	const RAShanHaiHuaDengBaseCfg * base_cfg = config->GetBaseCfg(ramgr, task_id);
	if (NULL == base_cfg)
	{
		return;
	}
	...
	{
	case SHANHAI_HUADENG_TASK_MODE_SINGLE_ENTER_SCORE:
		{
			param1 = (SHANHAI_HUADENG_TASK_TYPE_DAY == base_cfg->task_type) ? m_param.day_single_score_count : m_param.activity_single_score_count;
		}
		break;
	case SHANHAI_HUADENG_TASK_MODE_ADD_UP_SCORE:
		{
			param1 = (SHANHAI_HUADENG_TASK_TYPE_DAY == base_cfg->task_type) ? m_param.day_score_count : m_param.activity_score_count;
		}
		break;
	case SHANHAI_HUADENG_TASK_MODE_ENTER_NUM:
		{
			param1 = (SHANHAI_HUADENG_TASK_TYPE_DAY == base_cfg->task_type) ? m_param.day_join_times : m_param.activity_join_times;
		}
		break;
	default:
		return;
	...
	{
		if (m_role_module_mgr->GetKnapsack()->Put(base_cfg->reward_item, PUT_REASON_SHANHAI_HUADENG))
		{
			is_put_succ = true;
		}
	}
	if (!is_put_succ)
	{
		m_role_module_mgr->GetKnapsack()->SendMail(&base_cfg->reward_item, 1, SEND_MAIL_TYPE_ACTIVITY_REWARD, true, RAND_ACTIVITY_TYPE_SHANHAI_HUADENG);
	}
	...
	gamelog::g_log_burning_summer.printf(LL_INFO, "%s user[%d, %s], level[%d], activity_type[%d], task_id[%d], join_times[%d, %d] single_score[%d, %d], score_count[%d, %d]", __FUNCTION__,
		m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), m_role_module_mgr->GetRole()->GetLevel(), RAND_ACTIVITY_TYPE_SHANHAI_HUADENG, task_id,
		m_param.day_join_times, m_param.activity_join_times, m_param.day_single_score_count, m_param.activity_single_score_count, m_param.day_score_count, m_param.activity_score_count);
}

void RoleActivityShanHaiHuaDeng::OnForceClose()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
...
void RoleActivityShanHaiHuaDeng::OnFlowAway(int type)
{
	if (type < 0 || type >= MAX_RA_SHANHAI_HUADENG_LIGHT_TYPE_NUM)
	{
		return;
...
void RoleActivityShanHaiHuaDeng::OnRefreshLight(unsigned int now_second)
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	{
		if (light_list[i] < 0 || light_list[i] >= MAX_RA_SHANHAI_HUADENG_LIGHT_TYPE_NUM) continue;
...
void RoleActivityShanHaiHuaDeng::OnFinishGame()
{
	//山海初遇
...
void RoleActivityShanHaiHuaDeng::SendAllInfo()
{
	Protocol::SCRAShanHaiHuaDengAllInfo info;
...
void RoleActivityShanHaiHuaDeng::SendGameInfo(int notice_type)
{
	Protocol::SCRAShanHaiHuaDengGameInfo info;
...
void RoleActivityShanHaiHuaDeng::SendLightInfo()
{
	Protocol::SCRAShanHaiHuaDengLightInfo info;
	for (int i = 0; i < MAX_RA_SHANHAI_HUADENG_LIGHT_TYPE_NUM; i++)
	{
