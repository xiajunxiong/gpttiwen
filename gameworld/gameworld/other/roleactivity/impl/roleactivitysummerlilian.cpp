#include "roleactivitysummerlilian.hpp"
#include "gamelog.h"
...
RoleActivitySummerLiLian::RoleActivitySummerLiLian(int type)
	:RoleActivity(type)
{
}

RoleActivitySummerLiLian::~RoleActivitySummerLiLian()
{
}

void RoleActivitySummerLiLian::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(RASummerLiLianParam));
}

void RoleActivitySummerLiLian::GetInitParam(RoleActivityRoleData * p)
{
	memcpy(p->base_data, &m_param, sizeof(RASummerLiLianParam));
}

void RoleActivitySummerLiLian::OnLoginSendInfo()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;

	if (!ramgr->IsRandActivityOpen(RAND_ACTIVITY_TYPE_SUMMER_LI_LIAN))
	{
		return;
	}
	this->CheckInitRandActivity(RAND_ACTIVITY_TYPE_SUMMER_LI_LIAN);
	this->SendAllInfo();
}

void RoleActivitySummerLiLian::CheckInitRandActivity(int rand_act_type, bool is_force)
{
	if (RAND_ACTIVITY_TYPE_SUMMER_LI_LIAN != rand_act_type)
	{
		return;
	}
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	...
	if (is_force || !in_activity_time)
	{
		...
		m_param.open_day = LOGIC_CONFIG->GetRandActivityConfigManager()->GetActOpenDay(ramgr, RAND_ACTIVITY_TYPE_SUMMER_LI_LIAN);
...
void RoleActivitySummerLiLian::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
...
void RoleActivitySummerLiLian::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
	if (NULL == raor)
	{
		return;
	...
	{
		case Protocol::RA_SUMMER_LI_LIAN_REQ_TYPE_CHALLENGE:
		{
			this->OnChallenge(raor->param_1);
		}
		break;
		case Protocol::RA_SUMMER_LI_LIAN_REQ_TYPE_FETCH_ALL_PASS_REWARD:
		{
...
void RoleActivitySummerLiLian::OnBattleFinish(int activity_time, bool is_win)
{
	...
	const RASummerLiLianBaseCfg * base_cfg = config->GetBaseCfg(ramgr, activity_time);
	if (NULL == base_cfg)
	{
		return;
	}
	const RASummerLiLianRewardCfg * reward_cfg = config->GetRewardCfg(m_param.m_first_flag.IsBitSet(activity_time) ? base_cfg->common_reward_group_id : base_cfg->frist_reward_group_id);
	if (NULL == reward_cfg)
	{
		return;
	}
	...
	int add_exp = static_cast<int>((level_cfg->exp * reward_cfg->exp) / RASummerLiLianRewardCfg::EXP_BASE_NUM);
	if (add_exp > 0)
	{
	...
	if (!reward_cfg->reward_list.empty())
	{
		...
		{
			if (m_role_module_mgr->GetKnapsack()->PutList((short)reward_cfg->reward_list.size(), &reward_cfg->reward_list[0], PUT_REASON_SUMMER_LI_LIAN))
			{
				is_put_succ = true;
			}
		}
		if (!is_put_succ)
		{
			m_role_module_mgr->GetKnapsack()->SendMail(&reward_cfg->reward_list[0], (int)reward_cfg->reward_list.size(), SEND_MAIL_TYPE_ACTIVITY_REWARD, true, RAND_ACTIVITY_TYPE_SUMMER_LI_LIAN);
		}
	}
...
void RoleActivitySummerLiLian::SendAllInfo()
{
	Protocol::SCRASummerLiLianAllInfo info;
...
void RoleActivitySummerLiLian::OnChallenge(int activity_time)
{
	if(activity_time <= 0 || activity_time > MAX_RA_SUMMER_LI_LIAN_BOSS_NUM) return;
	...
	if (m_role_module_mgr->GetEscort()->HaveEscortTask())
	{
		m_role_module_mgr->NoticeNum(errornum::EN_NOW_ESCORTING_NO_START_CHANGE);
		return;
	}
	...
	const RASummerLiLianBossGroupCfg * boss_cfg = config->GetBossGroupCfg(base_cfg->boss_group_id, m_role_module_mgr->GetRole()->GetLevel());
	if (NULL == boss_cfg)
	{
		return;
	}
	...
	BattleManagerLocal::GetInstance().StartBattleModeFightReq(m_role_module_mgr->GetRole(), BATTLE_MODE_SUMMER_LI_LIAN, boss_cfg->monster_gourp_list[monster_index], 0, activity_time, true);

	this->OnParticipateRA(__FUNCTION__);
}

void RoleActivitySummerLiLian::OnFetchAllPassReward()
{
	RandActivityManager* ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const std::map<int, RASummerLiLianBaseCfg> * base_map = config->GetBaseMapCfg(ramgr);
	if (NULL == base_map)
	{
		return;
	}

	for (std::map<int, RASummerLiLianBaseCfg>::const_iterator it = base_map->begin(); it != base_map->end(); it++)
	{
	...
	const RASummerLiLianRewardCfg * reward_cfg = config->GetRewardCfg(other_cfg.all_pass_reward_group_id);
	if (NULL == reward_cfg)
	{
		return;
	}
...
bool RoleActivitySummerLiLian::IsLevelUnLock(int activity_time)
{
