...
#include "roleactivityxianshibaodai.hpp"


RoleActivityXianShiBaoDai::RoleActivityXianShiBaoDai(int type) :RoleActivity(type)
{
}

RoleActivityXianShiBaoDai::~RoleActivityXianShiBaoDai()
{
}

void RoleActivityXianShiBaoDai::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityXianShiBaoDai::GetInitParam(RoleActivityRoleData * p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityXianShiBaoDai::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityXianShiBaoDai::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
...
void RoleActivityXianShiBaoDai::CheckInitRandActivity(int rand_act_type, bool is_force)
{
...
bool RoleActivityXianShiBaoDai::IsFinishBonusCond(int type)
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	const RandActivityXianShiBaoDaiConfig * config = static_cast<const RandActivityXianShiBaoDaiConfig *>
...
void RoleActivityXianShiBaoDai::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
	switch (raor->opera_type)
	{
		...
		case 2:
			{
				this->ReceiveFreeReward();			// 限时宝袋免费奖励
			}
			break;
	}
}

bool RoleActivityXianShiBaoDai::OnBuyItemOld(int seq, int chong_zhi_gold)
{
	...
	const RAXianShiBaoDaiItemCfg * buy_cfg = config->GetRAXianShiBaoDaiItemCfg(ramgr, seq);
	if (buy_cfg == NULL)
	{
		return false;
	}
...
int RoleActivityXianShiBaoDai::OnBuyGiftItem(int seq, ARG_OUT int * price)
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_DAI_MAX_ITEM)
	{
		return 1;
	...
	if (!ramgr->IsRandActivityOpen(RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_DAI))
	{
		return 1;
	}
...
int RoleActivityXianShiBaoDai::OnBuyItemCheck(int seq, long long ARG_OUT & unique_key)
{
	unique_key = 0;
	...
	CmdBuyRecordManager::Instance().DelRedundantCmdBuyRecord< RoleCommonSaveCmdBuyItem >(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, gamelog::g_log_xian_shi_bao_dai, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();

	std::string log_str2 = ::GetString(*buy_item);
	gamelog::g_log_xian_shi_bao_dai.printf(LL_INFO, "%s line:%d |  role[%d %s] %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), log_str2.c_str());
	
	return 0;
}

int RoleActivityXianShiBaoDai::OnBuyItem(int chong_zhi_gold, long long unique_id)
{
	...
	gamelog::g_log_xian_shi_bao_dai.printf(LL_INFO, "%s line:%d | role[%d %s] ra_begin_timestamp:%u %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(),
	...
	CmdBuyRecordManager::Instance().DelCmdBuyRecordByUniqueId<RoleCommonSaveCmdBuyItem>(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, unique_id, gamelog::g_log_xian_shi_bao_dai, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();
...
void RoleActivityXianShiBaoDai::SendInfo()
{
	Protocol::SCRAXianShiBaoDaiInfo protocol;
...
void RoleActivityXianShiBaoDai::ReceiveFreeReward()
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const RAXianShiBaoDaiOtherCfg & other_cfg = config->GetOtherCfg();
