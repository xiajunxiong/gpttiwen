...
#include "roleactivityxianshibaohe2.hpp"


RoleActivityXianShiBaoHe2::RoleActivityXianShiBaoHe2(int type) :RoleActivity(type)
{
}

RoleActivityXianShiBaoHe2::~RoleActivityXianShiBaoHe2()
{
}

void RoleActivityXianShiBaoHe2::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityXianShiBaoHe2::GetInitParam(RoleActivityRoleData * p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityXianShiBaoHe2::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityXianShiBaoHe2::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
...
void RoleActivityXianShiBaoHe2::CheckInitRandActivity(int rand_act_type, bool is_force)
{
...
bool RoleActivityXianShiBaoHe2::IsFinishBonusCond(int type)
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr)
	{
	...
	const RandActivityXianShiBaoHe2Config * config = static_cast<const RandActivityXianShiBaoHe2Config *>
	...
	bool is_finish = true;
	for (int i = 0; i < item_num && i < RA_XIAN_SHI_BAO_HE_2_BUY_MAX_ITEM; i++)
	{
...
void RoleActivityXianShiBaoHe2::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
...
bool RoleActivityXianShiBaoHe2::OnBuyItemOld(int seq, int chong_zhi_gold)
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_HE_2_MAX_ITEM)
	{
		return false;
	}
	...
	const RAXianShiBaoHe2ItemCfg * buy_cfg = config->GetRAXianShiBaoHeItemCfg(ramgr, seq);
	if (buy_cfg == NULL)
	{
		return false;
	}
...
int RoleActivityXianShiBaoHe2::OnBuyGiftItem(int seq, ARG_OUT int * price)
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_HE_2_MAX_ITEM)
	{
		return 1;
	...
	if (!ramgr->IsRandActivityOpen(RAND_ACTIVITY_TYPE_XIAN_SHI_BAO_HE_2))
	{
		return 1;
	}
...
int RoleActivityXianShiBaoHe2::OnBuyItemCheck(int seq, long long ARG_OUT & unique_key)
{
	unique_key = 0;
	...
	CmdBuyRecordManager::Instance().DelRedundantCmdBuyRecord< RoleCommonSaveCmdBuyItem >(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, gamelog::g_log_xian_shi_bao_he_2, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();

	std::string log_str2 = ::GetString(*buy_item);
	gamelog::g_log_xian_shi_bao_he_2.printf(LL_INFO, "%s line:%d |  role[%d %s] %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), log_str2.c_str());

	return 0;
}

int RoleActivityXianShiBaoHe2::OnBuyItem(int chong_zhi_gold, long long unique_id)
{
	...
	gamelog::g_log_xian_shi_bao_he_2.printf(LL_INFO, "%s line:%d | role[%d %s] ra_begin_timestamp:%u %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(),
	...
	{
		if (0 <= seq && seq < RA_XIAN_SHI_BAO_HE_2_MAX_ITEM)
		{
			if (limit_type == 1)
			{
				m_param.ra_buy_times[seq] += 1;
	...
	CmdBuyRecordManager::Instance().DelCmdBuyRecordByUniqueId<RoleCommonSaveCmdBuyItem>(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, unique_id, gamelog::g_log_xian_shi_bao_he_2, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();
...
void RoleActivityXianShiBaoHe2::SendInfo()
{
	Protocol::SCRAXianShiBaoHe2Info protocol;
...
void RoleActivityXianShiBaoHe2::ReceiveFreeReward()
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const RAXianShiBaoHe2OtherCfg & other_cfg = config->GetOtherCfg();
