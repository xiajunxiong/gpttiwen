...
#include "roleactivityxianshibaohe3.hpp"


RoleActivityXianShiBaoHe3::RoleActivityXianShiBaoHe3(int type) :RoleActivity(type)
{
}

RoleActivityXianShiBaoHe3::~RoleActivityXianShiBaoHe3()
{
}

void RoleActivityXianShiBaoHe3::Init(RoleModuleManager * mgr, const RoleActivityRoleData & p)
{
	m_role_module_mgr = mgr;
	memcpy(&m_param, p.base_data, sizeof(m_param));
}

void RoleActivityXianShiBaoHe3::GetInitParam(RoleActivityRoleData * p)
{
	UNSTD_STATIC_CHECK(sizeof(m_param) <= sizeof(p->base_data));
	memcpy(p->base_data, &m_param, sizeof(m_param));
}

void RoleActivityXianShiBaoHe3::OnLoginSendInfo()
{
	this->SendInfo();
}

void RoleActivityXianShiBaoHe3::OnDayChange(unsigned int old_dayid, unsigned int now_dayid)
{
...
void RoleActivityXianShiBaoHe3::CheckInitRandActivity(int rand_act_type, bool is_force)
{
...
void RoleActivityXianShiBaoHe3::OnRandActivityOpReq(Protocol::CSRandActivityOperaReq * raor)
{
...
int RoleActivityXianShiBaoHe3::OnBuyGiftItem(int seq, ARG_OUT int * price)
{
	if (seq < 0 || seq >= RA_XIAN_SHI_BAO_HE_3_MAX_ITEM)
	{
	...
	const RandActivityXianShiBaoHe3Config * config = static_cast<const RandActivityXianShiBaoHe3Config *>
	...
	const RAXianShiBaoHe3ItemCfg * buy_cfg = config->GetRAXianShiBaoHeItemCfg(ramgr, seq);
	if (buy_cfg == NULL)
	{
		return BUY_GIFT_ITEM_RET_TYPE_1;
	}
	...
	{
		m_role_module_mgr->GetKnapsack()->PutVec(buy_cfg->rewards, PUT_REASON_XIAN_SHI_BAO_HE_3);
	}
	else
	{
		// 发邮件
...
int RoleActivityXianShiBaoHe3::OnBuyItemCheck(int seq, long long ARG_OUT & unique_key)
{
	unique_key = 0;
	...
	CmdBuyRecordManager::Instance().DelRedundantCmdBuyRecord< RoleCommonSaveCmdBuyItem >(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, gamelog::g_log_xian_shi_bao_he_3, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();

	std::string log_str3 = ::GetString(*buy_item);
	gamelog::g_log_xian_shi_bao_he_3.printf(LL_INFO, "%s line:%d |  role[%d %s] %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(), log_str3.c_str());

	return 0;
}

int RoleActivityXianShiBaoHe3::OnBuyItem(int chong_zhi_gold, long long unique_id)
{
	...
	gamelog::g_log_xian_shi_bao_he_3.printf(LL_INFO, "%s line:%d | role[%d %s] ra_begin_timestamp:%u %s", __FUNCTION__, __LINE__, m_role_module_mgr->GetRole()->GetUID(), m_role_module_mgr->GetRole()->GetName(),
	...
	{
		if (0 <= seq && seq < RA_XIAN_SHI_BAO_HE_3_MAX_ITEM)
		{
	...
	CmdBuyRecordManager::Instance().DelCmdBuyRecordByUniqueId<RoleCommonSaveCmdBuyItem>(m_role_module_mgr->GetRole(), COMMON_SAVE_TYPE, unique_id, gamelog::g_log_xian_shi_bao_he_3, __FUNCTION__);
	m_role_module_mgr->GetRole()->Save();
...
void RoleActivityXianShiBaoHe3::SendInfo()
{
	Protocol::SCRAXianShiBaoHe3Info protocol;
...
void RoleActivityXianShiBaoHe3::ReceiveFreeReward()
{
	RandActivityManager * ramgr = RandActivityAdapter::Instance()->GetRAManager(m_role_module_mgr->GetRole()->GetUniqueServerID());
	if (NULL == ramgr) return;
	...
	const RAXianShiBaoHe3OtherCfg & other_cfg = config->GetOtherCfg();
	...
	gamelog::g_log_xian_shi_bao_he_3.printf(LL_INFO, "::%s user[%d, %s] get_reward[%s]", __FUNCTION__,
