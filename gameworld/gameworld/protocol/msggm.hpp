...
namespace Protocol
{
	...
	// GM命令 
	class CSGMCommand  //!< 2050 GM命令请求
	{
	public:
		CSGMCommand();
		MessageHeader	header;
		char type[MAX_GM_COMMAND_TYPE_LEN];		//!< \see MAX_GM_COMMAND_TYPE_LEN
		char command[MAX_GM_COMMAND_LEN];		//!< \see MAX_GM_COMMAND_LEN
	};

	//GM命令回复
	class SCGMCommand  //!< 2000 GM命令返回
	{
	public:
		SCGMCommand();
		MessageHeader	header;
		char type[MAX_GM_COMMAND_TYPE_LEN];		//!< \see MAX_GM_COMMAND_TYPE_LEN
		char result[MAX_GM_COMMAND_LEN];		//!< \see MAX_GM_COMMAND_LEN
	};
