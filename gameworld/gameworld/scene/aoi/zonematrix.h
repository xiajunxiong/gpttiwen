...
public:
	...
	bool IsPosInSight(UInt32 aoi_handler, const Posi& posi);

	void NotifyAreaMsg(UInt32 obser_handle, void *msg, unsigned int length);

	int GetObjByArea(Posi pos, Axis x_range, Axis y_range, ObjID *obj_list, int max_num);
	int GetObjByCircle(Posi pos, Axis range, ObjID *obj_list, int max_num);
	int GetObjBySector(Posi pos_zero, Posi pos_target, Axis range, double angle, ObjID *obj_list, int max_num); // 以zero为原点， 以target为目标，半径为range，角度为angle(0,360)的扇形 ( 即zero与target的连线把扇形一分为二 ）
	int GetObjByRectangle(Posi pos_zero, Posi pos_target, Axis range, double width, ObjID *obj_list, int max_num); // 以zero为原点， 以target为目标，长度为range, 宽为width的矩形
protected:
