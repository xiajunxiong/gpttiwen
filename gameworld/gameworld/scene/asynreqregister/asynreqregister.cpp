...
bool AsynReqRegister::OnAsynRspCheckNetidExist(GSNetID netid, int key, int role_id)
{
	...
	if (it != m_gsnetid_set.end())
	{
		...
		{
			gamelog::g_log_login.printf(LL_ERROR, "[ERROR AsynReqRegister::OnAsynRspCheckNetidExist netid exist bug key not match [%d,%d],  roleid[%d] ]", netid.gateway_netid, netid.netid, role_id);
		}
	}
	else 
	{
		gamelog::g_log_login.printf(LL_ERROR, "[ERROR AsynReqRegister::OnAsynRspCheckNetidExist netid not exist[%d,%d],  roleid[%d]", netid.gateway_netid, netid.netid, role_id);
	}
