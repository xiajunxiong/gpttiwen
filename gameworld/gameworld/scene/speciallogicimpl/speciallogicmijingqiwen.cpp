#include "speciallogicmijingqiwen.hpp"
...
SpecialLogicMiJingQiWenFB::SpecialLogicMiJingQiWenFB(Scene * scene) : SpecialLogic(scene), m_is_init(false), m_layer(false), m_level_seq(0), m_monster_num(0),
...
SpecialLogicMiJingQiWenFB::~SpecialLogicMiJingQiWenFB()
{
}

void SpecialLogicMiJingQiWenFB::Update(unsigned long interval, time_t now_second)
{
...
void SpecialLogicMiJingQiWenFB::OnRoleEnterScene(Role *role)
{
	if (NULL == role) return;
...
void SpecialLogicMiJingQiWenFB::OnRoleLeaveScene(Role *role, bool is_logout)
{
	role->RestoreLastEncounterMonsterFlag();
}

void SpecialLogicMiJingQiWenFB::OnFightFinish(battlegameprotocol::BattleGameFightResultAck* ack)
{
	if (BATTLE_MODE_RA_MIJINGQIWEN == ack->battle_mode)
	{
		...
		if (BATTLE_MODE_RA_MIJINGQIWEN == ack->battle_mode && RESULT_TYPE_ATTACKER_WIN == ack->battle_result)
		{
			...
			if (NULL != npc && Obj::OBJ_TYPE_NPC_MONSTER == npc->GetObjType())
			{
				...
				int monster_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_MONSTER;
				bool is_boss = npc_monster->GetId() == m_boss_obj_id;

				if (!is_boss)
				{
					const MiJingQiWenFbNPCRewardCfg * npc_cfg = config->GetNpcRewardCfg(npc_monster->GetNpcId(), ack->business_data.param1);
					if (NULL != npc_cfg)
					{
						if (npc_cfg->is_delete == 1)
						{
							std::map<int, std::vector<SpecialLogicCrystalObjInfo> >::iterator it = m_obj_info_list.find(Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_MONSTER);
				...
				else
				{
					...
					for (int i = 0; i < ack->attacker_list.role_count && i < SIDE_MAX_ROLE_NUM; ++i)
					{
						...
						DropPool::Instance()->DropListToRole(temp_role, drop_id_list, ack->monster_group_id, true, npc_monster->GetId() == m_boss_obj_id ?
							GET_REASON_MIJIQIWEN : GET_REASON_INVALID, 1, NULL, 0, npc_monster->GetNpcId(), 0, RAND_ACTIVITY_TYPE_ANECDOTE);
						time_t & node = kick_out_tamp[temp_role->GetUID()];
						node = EngineAdapter::Instance().Time() + 10;
					}
					m_boss_obj_id = INVALID_OBJ_ID;
					monster_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_BOSS;
				...
				if (monster_type != Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_BOSS)
				{
					m_monster_num -= 1;
				}
			...
			else if (NULL != npc && Obj::OBJ_TYPE_NPC_CHOSED == npc->GetObjType())
			{
				...
				int monster_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_NPC;
		...
		else if (BATTLE_MODE_RA_MIJINGQIWEN == ack->battle_mode && RESULT_TYPE_DEFENDER_WIN == ack->battle_result)
		{
			//怪物消失，并且赋予玩家负面buff
			int npc_seq_id = ack->business_data.monster_gen_id;
			NPC* npc = NpcManager::Instance().GetServerNPC(npc_seq_id);
			if (NULL != npc && Obj::OBJ_TYPE_NPC_MONSTER == npc->GetObjType())
			{
				...
				else
				{
					...
					for (int i = 0; i < ack->attacker_list.role_count && i < SIDE_MAX_ROLE_NUM; ++i)
					{
						...
						DropPool::Instance()->DropListToRole(temp_role, drop_id_list, ack->monster_group_id, true, npc_monster->GetId() == m_boss_obj_id ?
							GET_REASON_MIJIQIWEN : GET_REASON_INVALID, 1, NULL, 0, npc_monster->GetNpcId(), 0, RAND_ACTIVITY_TYPE_ANECDOTE, false);
						time_t & node = kick_out_tamp[temp_role->GetUID()];
						node = EngineAdapter::Instance().Time() + 10;
					}
					m_boss_obj_id = INVALID_OBJ_ID;
			...
			else if (NULL != npc && Obj::OBJ_TYPE_NPC_CHOSED == npc->GetObjType())
			{
				...
				const MiJingQiWenFbNPCRewardCfg * npc_cfg = config->GetNpcRewardCfg(npc_chose->GetNpcId(), ack->business_data.param3);
				if (NULL != npc_cfg)
				{
					if (npc_cfg->is_delete == 1)
					{
						std::map<int, std::vector<SpecialLogicCrystalObjInfo> >::iterator it = m_obj_info_list.find(Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_NPC);
						if (it != m_obj_info_list.end())
						{
							for (std::vector<SpecialLogicCrystalObjInfo>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++)
							{
...
void SpecialLogicMiJingQiWenFB::OnRoleRunAway(battlegameprotocol::BattleGameRoleRunAway* ack)
{
	if (BATTLE_MODE_RA_MIJINGQIWEN == ack->battle_mode)
	{
		const RandActivityMiJingQiWenConfig * config = static_cast<const RandActivityMiJingQiWenConfig *>
...
void SpecialLogicMiJingQiWenFB::OnBattleForcedFinish(battlegameprotocol::BattleGameForceFinishFight* ack)
{
	if (BATTLE_MODE_RA_MIJINGQIWEN == ack->battle_mode)
	{
		const RandActivityMiJingQiWenConfig * config = static_cast<const RandActivityMiJingQiWenConfig *>
...
bool SpecialLogicMiJingQiWenFB::IsForceEncounterMineMonster()
{
	return !this->IsLastLayer(m_layer);
}

bool SpecialLogicMiJingQiWenFB::CanEncounterMineMonster(Role * role)
{
	return !this->IsLastLayer(m_layer);
}

bool SpecialLogicMiJingQiWenFB::CanUseItem(Role * role, ItemID item_id, int num)
{
...
void SpecialLogicMiJingQiWenFB::OnUseItem(Role * role, ItemID item_id, int num)
{
	const ItemBase *item_base = ITEMPOOL->GetItem(item_id);
	if (NULL == item_base)
	{
...
void SpecialLogicMiJingQiWenFB::OnTransport(Role * role, SpecialTransport * transport_obj)
{
...
void SpecialLogicMiJingQiWenFB::OnRoleLeaveTeam(Role * role)
{
	this->DelayKickOutRole(role);
}

void SpecialLogicMiJingQiWenFB::OnNPCChosedReward(Role * role, ObjID npc_objid, int option_index)
{
	const RandActivityMiJingQiWenConfig * config = static_cast<const RandActivityMiJingQiWenConfig *>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(RAND_ACTIVITY_TYPE_ANECDOTE));
	if (NULL == config)
	...
	if (npc_obj != NULL && Obj::OBJ_TYPE_NPC_CHOSED == npc_obj->GetObjType())
	{
		...
		const MiJingQiWenFbNPCRewardCfg * npc_cfg = config->GetNpcRewardCfg(npc_chose->GetNpcId(), option_index);
		if (NULL != npc_cfg)
		{
			...
			if (npc_cfg->is_delete == 1)
			{
				...
				if (is_find)
				{
					...
					if (npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_2)
					{
						m_npc_num -= 1;
					}
					else if (npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_4)
					{
						m_gouhuo_num -= 1;
					}
					this->SendSceneObjInfo();
...
void SpecialLogicMiJingQiWenFB::OnGather(Role * role, int gather_id, int seq_id, const Posi & pos)
{
	NPC *npc = NULL;
					...
					case GatherNpcConfig::REWARD_TYPE_1:
						{
							...
							if (reward_cfg != NULL)
							{
								...
								{
									gamelog::g_log_knapsack.printf(LL_INFO, "SpecialLogicMiJingQiWenFB::OnGather role[%s,%d] knapsack full no put item[id:%d,num:%d,is_bind:%d]",
										member_role->GetName(), member_role->GetUID(), reward_cfg->item_id, reward_cfg->num, reward_cfg->is_bind ? 1 : 0);
								}
								else
								{
									RoleActivity * role_activity = member_role->GetRoleModuleManager()->GetRoleActivityManager()->GetRoleActivity(RAND_ACTIVITY_TYPE_ANECDOTE);
					...
					case GatherNpcConfig::REWARD_TYPE_2:
						{
							...
							if (!member_role->GetRoleModuleManager()->GetMoney()->AddCoinBind(real_coin, "SpecialLogicMiJingQiWenFB::OnGather"))
							{
								return;
							}
							else
			...
			case GatherNpcConfig::REWARD_TYPE_1:
				{
					...
					if (reward_cfg != NULL)
					{
						...
						{
							RoleActivity * role_activity = role->GetRoleModuleManager()->GetRoleActivityManager()->GetRoleActivity(RAND_ACTIVITY_TYPE_ANECDOTE);
							if (NULL != role_activity)
							{
								std::vector<ItemConfigData> item_list; item_list.push_back(put_reward);
								role_activity->OnSendResultNotice(Protocol::SCRAResultNotice::RAND_ACTIVITY_RESULT_NOTICE_TYPE_GET_REWARD, &item_list);
			...
			case GatherNpcConfig::REWARD_TYPE_2:
				{
					...
					if (!role->GetRoleModuleManager()->GetMoney()->AddCoinBind(real_coin, "SpecialLogicMiJingQiWenFB::OnGather"))
					{
						return;
					}
					else
		...
		if (server_npc_cfg != NULL)
		{
			for (std::map<int, std::vector<SpecialLogicCrystalObjInfo> >::iterator it = m_obj_info_list.begin(); it != m_obj_info_list.end(); it++)
			{
				for (std::vector<SpecialLogicCrystalObjInfo>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++)
				{
					...
					{
						if (it2->obj_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_6)
						{
							m_gather_num -= 1;
						}
						else if (it2->obj_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_8)
						{
							m_gather_num -= 1;
						}
						else if (it2->obj_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_7)
						{
							m_gather_num -= 1;
						}
						else if (it2->obj_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_5)
						{
							m_coin_num -= 1;
						}
...
void SpecialLogicMiJingQiWenFB::OnNPCChosedTalk(Role * role, ObjID npc_objid)
{
	Obj * npc_obj = m_scene->GetObj(npc_objid);
	if (npc_obj != NULL && Obj::OBJ_TYPE_NPC_CHOSED == npc_obj->GetObjType())
	{
		...
		for (std::map<int, std::vector<SpecialLogicCrystalObjInfo> >::iterator it = m_obj_info_list.begin(); it != m_obj_info_list.end(); it++)
		{
			for (std::vector<SpecialLogicCrystalObjInfo>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++)
			{
				if (it2->obj_id == npc_objid)
				{
					if (it2->obj_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_4)not_gouhuo = true;
				}
			}
		}
...
void SpecialLogicMiJingQiWenFB::OnRoleAnswerQuestion(Role * role, ObjID npc_objid, int answer_id)
{
	if (role->InTeam())
	{
	...
	std::map<ObjID, Protocol::MiJingQuestion>::iterator it = m_question.find(npc_objid);
	...
	const MiJingQiWenFbNPCRewardCfg * npc_cfg = config->GetNpcRewardCfg(npc_chosed->GetNpcId(), 0);
	if (npc_cfg == NULL)return;
	const MiJingQiWenFbNPCRewardListCfg *npc_event = config->GetNpcEventCfg(npc_chosed->GetNpcId());
	...
	if (answer_ret)
	{
		...
		if (npc_event->reward_list[0].reward_type == MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_5)
		{
			reward_buff_id = answer_ret ? npc_event->reward_list[0].reward_list[0] : npc_event->reward_list[0].reward_list[1];
		}
		else if (npc_event->reward_list[1].reward_type == MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_5)
		{
...
void SpecialLogicMiJingQiWenFB::OnRoleBuyItem(Role * role, ObjID npc_objid, int item_idx)
{
	std::map<int, MiJingItemInfo>::iterator it = m_shop_info.shop_info.find(item_idx);
	...
	//检查次数
	BuyList &user_info = scene_user_shop_map[role->GetUID()];
	...
	{
	case MiJingItemInfo::PRICE_TYPE_0:
		{
		...
		break;
	case MiJingItemInfo::PRICE_TYPE_2:
		{
...
void SpecialLogicMiJingQiWenFB::SetInfo(int layer, int level_seq, Posi transport_pos, std::vector<new_Event_NPC_cfg> * has_create_npc_vec, std::map<int, std::vector<ItemConfigData> >* item_list)
{
	const RandActivityMiJingQiWenConfig * config = static_cast<const RandActivityMiJingQiWenConfig *>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(RAND_ACTIVITY_TYPE_ANECDOTE));
	if (NULL == config)
...
void SpecialLogicMiJingQiWenFB::SendSceneInfo(Role * role)
{
	Protocol::SCMiJingQiWenFbObjInfo scene_info;
	...
	for (std::map<int, std::vector<SpecialLogicCrystalObjInfo> >::iterator it = m_obj_info_list.begin(); it != m_obj_info_list.end(); it++)
	{
		if ((int)it->second.size() > 0 && scene_info.obj_count < Protocol::SCMiJingQiWenFbObjInfo::MAX_OBJ_LIST_COUNT)
		{
			for (std::vector<SpecialLogicCrystalObjInfo>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++)
			{
...
void SpecialLogicMiJingQiWenFB::SendSceneObjInfo(Role * role)
{
	Protocol::SCMiJingQiWenFbObjInfo scene_info;
	scene_info.scene_id = this->GetScene()->GetSceneID();
	scene_info.level_seq = m_level_seq;
	...
	if (NULL != role)
	{
		EngineAdapter::Instance().NetSend(role->GetNetId(), (const char *)&scene_info, sizeof(scene_info) - ((Protocol::SCMiJingQiWenFbObjInfo::MAX_OBJ_LIST_COUNT - scene_info.obj_count) * sizeof(scene_info.obj_list[0])));
	}
	else
	{
		m_scene->SendToRole((const char*)&scene_info, sizeof(scene_info));
...
void SpecialLogicMiJingQiWenFB::SendQuestionInfo(ObjID npc_objid, Role * role)
{
	Protocol::SCRAMiJingQiWenQuestionInfo info;
	std::map<ObjID, Protocol::MiJingQuestion>::iterator it = m_question.find(npc_objid);
	if (it == m_question.end())return;
...
void SpecialLogicMiJingQiWenFB::SendShopInfo(Role * role)
{
	Protocol::SCRAMiJingQiWenShopInfo info;
	info.shop_num = 0;
	std::map<UserID, BuyList>::iterator buy_it = scene_user_shop_map.find(role->GetUID());
	for (int i = 0; i < Protocol::SCRAMiJingQiWenShopInfo::MAX_SHOP_ITEM_NUM && i < (int)m_shop_info.shop_info.size(); i++)
	{
		std::map<int, MiJingItemInfo>::iterator it = m_shop_info.shop_info.find(i);
		if (it != m_shop_info.shop_info.end())
		{
			...
			{
				if (it->first >= 0 && it->first < Protocol::SCRAMiJingQiWenShopInfo::MAX_SHOP_ITEM_NUM)
				{
	...
	EngineAdapter::Instance().NetSend(role->GetNetId(), (const char *)&info, sizeof(info) - ((Protocol::SCRAMiJingQiWenShopInfo::MAX_SHOP_ITEM_NUM - info.shop_num) * sizeof(Protocol::SCRAMiJingQiWenShopInfo::ItemInfo)));
}

void SpecialLogicMiJingQiWenFB::CreateNpc()
{
	const RandActivityMiJingQiWenConfig * config = static_cast<const RandActivityMiJingQiWenConfig *>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(RAND_ACTIVITY_TYPE_ANECDOTE));
	if (NULL == config)
	...
	m_coin_num = 0;
	m_gouhuo_num = 0;
	std::vector<new_Event_NPC_cfg> npc_node_cfg;
			...
			case SERVER_NPC_TYPE_GATHERS:
				{
					...
					if (npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_6)
					{
						node_info.obj_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_GATHER0;
						m_obj_info_list[Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_GATHER0].push_back(node_info);
						m_gather_num += 1;
					}
					else if (npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_8)
					{
						node_info.obj_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_GATHER2;
						m_obj_info_list[Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_GATHER2].push_back(node_info);
						m_gather_num += 1;
					}
					else if (npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_7)
					{
						node_info.obj_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_GATHER1;
						m_obj_info_list[Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_GATHER1].push_back(node_info);
						m_gather_num += 1;
					}
					else if (npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_5)
					{
						node_info.obj_type = Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_COIB;
						m_obj_info_list[Protocol::SCMiJingQiWenFbObjInfo::MI_JING_QI_WEN_FB_OBJ_TYPE_COIB].push_back(node_info);
						m_coin_num += 1;
					}
				}
				break;
			case SERVER_NPC_TYPE_CHOSED:
				{
					...
					if (NULL != monster)
					{
						monster->SetBattleMode(BATTLE_MODE_RA_MIJINGQIWEN);
					}
					//检查事件
					const MiJingQiWenFbNPCRewardListCfg *npc_event = config->GetNpcEventCfg(npc_node_cfg[i].npc_id);
					if (npc_event != NULL)
					{
						m_has_create_npc_vec.push_back(npc_node_cfg[i]);//记录已经创建过的交互NPC
						int shop_id = 0;
						if (npc_event->reward_list[0].reward_type == MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_2)
						{
							shop_id = npc_event->reward_list[0].reward_list[0];
						}
						else if (npc_event->reward_list[1].reward_type == MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_2)
						{
						...
						if (shop_id != 0)//是商店，导入对应商店信息
						{
							...
							{
								MiJingItemInfo &node = m_shop_info.shop_info[i];
					...
					if (npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_4)
					{
						node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_4;
						m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_4].push_back(node_info);
						m_gouhuo_num += 1;
					}
					else if (npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_2)
					{
						node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_2;
						m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_2].push_back(node_info);
						m_npc_num += 1;
					}
					else if(npc_node_cfg[i].npc_type == new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_11)
					{
						//随机题目
						const MiJingQiWenOtherCfg & other_cfg = config->GetCrystalFbOtherCfg();
						...
						if (NULL != question_group_cfg)
						{
							Protocol::MiJingQuestion &node = m_question[node_info.obj_id];
						...
						node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_11;
						m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_11].push_back(node_info);
						m_npc_num += 1;
					}
				}
				break;
	...
	if (this->IsLastLayer(m_layer))
	{
		...
		NPCChosed * monster = NpcManager::Instance().CreateNPCChosed(cfg->recover_npc_id, m_scene, cfg->recover_npc_pos, BATTLE_MODE_RA_MIJINGQIWEN, &cfg->recover_npc_dir);
...
void SpecialLogicMiJingQiWenFB::CreateMonster()
{
	const RandActivityMiJingQiWenConfig * config = static_cast<const RandActivityMiJingQiWenConfig *>
		(LOGIC_CONFIG->GetRandActivityConfigManager()->GetRandActivityConfig(RAND_ACTIVITY_TYPE_ANECDOTE));
	if (NULL == config)
	...
	{
		NPCMonster * monster = NpcManager::Instance().CreateNPCMonster(cfg->boss_npc_id, m_scene, cfg->boss_pos, BATTLE_MODE_RA_MIJINGQIWEN, &cfg->monster_dir);
		if (NULL != monster)
		{
			...
			node_info.npc_id = monster->GetNpcId();
			node_info.obj_id = monster->GetId();
			node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_3;
			node_info.pos_x = monster->GetPos().x;
			node_info.pos_y = monster->GetPos().y;
			m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_3].push_back(node_info);
		}
	}
	
	{
		...
		for (int i = 0; i < monster_num && i < 100; ++i)//避免无限判空死循环
		{
			...
			NPCMonster * monster = NpcManager::Instance().CreateNPCMonster(monster_cfg->monster_npc_id, m_scene, m_scene->GetARandPosi(), BATTLE_MODE_RA_MIJINGQIWEN, &cfg->monster_dir);
			if (NULL != monster)
			{
				m_monster_num += 1;
				SpecialLogicCrystalObjInfo node_info;
				node_info.npc_id = monster->GetNpcId();
				node_info.obj_id = monster->GetId();
				node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_1;
				node_info.pos_x = monster->GetPos().x;
				node_info.pos_y = monster->GetPos().y;
				m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_1].push_back(node_info);
			}
		}
	}
}

void SpecialLogicMiJingQiWenFB::CreateBox()
{
	if (this->IsLastLayer(m_layer))
	{
		return;
	...
	for (int i = 0; i < box_num; ++i)
	{
		...
		node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_9;
		node_info.pos_x = treasure_chest_obj->GetPos().x;
		node_info.pos_y = treasure_chest_obj->GetPos().y;
		m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_9].push_back(node_info);
	}

}

void SpecialLogicMiJingQiWenFB::CreateTransport()
{
	if (this->IsLastLayer(m_layer))
	{
		return;
	...
	if (transport_obj != NULL)
	{
		...
		node_info.obj_type = new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_10;
		node_info.pos_x = transport_obj->GetPos().x;
		node_info.pos_y = transport_obj->GetPos().y;
		m_obj_info_list[new_Event_NPC_cfg::CRYSTALFB_NPC_TYPE_10].push_back(node_info);
	}
}

void SpecialLogicMiJingQiWenFB::CreateLeaveTransport(const Posi & pos)
{
	SpecialTransport * transport_obj = new SpecialTransport();
...
bool SpecialLogicMiJingQiWenFB::IsLastLayer(int layer)
{
	return true;//必然是最后一层
}

void SpecialLogicMiJingQiWenFB::TransportNextLayer(Role * role, SpecialTransport * transport_obj)
{
	if (NULL == role || NULL == transport_obj)
	{
		return;
	...
	SpecialLogicMiJingQiWenFB * special_lobgic = World::GetInstWorld()->GetSceneManager()->GetSpecialLogicMiJingQiWenFB(scene_id, scene_key);
...
void SpecialLogicMiJingQiWenFB::RoleAloneGetRewardByCfg(Role * role, const MiJingQiWenFbNPCRewardCfg * npc_cfg, ObjID *targer_obj)
{
	...
	{
	case MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_0:
		{
		...
		break;
	case MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_1:
		{
			...
			if (reward_cfg != NULL)
			{
				...
				{
					gamelog::g_log_knapsack.printf(LL_INFO, "SpecialLogicMiJingQiWenFB::OnNPCChosedReward role[%s,%d] knapsack full no put item[id:%d,num:%d,is_bind:%d]",
						role->GetName(), role->GetUID(), reward_cfg->item_id, reward_cfg->num, reward_cfg->is_bind ? 1 : 0);
				}
			}
		}
		break;
	case MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_2:
		{
			this->SendShopInfo(role);
		}
		break;
	case MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_3:
		{
			if (npc_cfg->reward_list[index] > 0)
			{
				if (!role->GetRoleModuleManager()->GetMoney()->AddCoinBind(npc_cfg->reward_list[index], "SpecialLogicMiJingQiWenFB::OnNPCChosedReward"))
				{
					gamelog::g_log_knapsack.printf(LL_INFO, "SpecialLogicMiJingQiWenFB::OnNPCChosedReward role[%s,%d] knapsack add bindcoin[%d]",
		...
		break;
	case MiJingQiWenFbNPCRewardCfg::REWARD_TYPE_5:
		{
...
void SpecialLogicMiJingQiWenFB::RoleTeamGetRewardByCfg(Role * role, const MiJingQiWenFbNPCRewardCfg * npc_cfg, ObjID *targer_obj)
{
	if (role == NULL || npc_cfg == NULL)return;
	if (role->InTeam())
	{
...
void SpecialLogicMiJingQiWenFB::AddRoleBuff(Role * role, const BuffListConfig * buff_cfg)
{
...
void SpecialLogicMiJingQiWenFB::SendBuffEffectToRole(Role * buff_geter, int buff_type)
{
...
void SpecialLogicMiJingQiWenFB::TeamGoNextLayer(Team * team, int target_scene_id, int target_scene_key, const Posi & target_pos)
{
	if (NULL == team) return;
...
void SpecialLogicMiJingQiWenFB::ItemStack(Role * role, const ItemConfigData * item)
{
