#ifndef __SPECIAL_LOGIC_MI_JING_QI_WEN_2_HPP__
#define __SPECIAL_LOGIC_MI_JING_QI_WEN_2_HPP__
...
struct BuyList2
{
	...
	BuyInfo2 m_buy_info[Protocol::SCRAMiJingQiWenShopInfo2::MAX_SHOP_ITEM_NUM];
};

struct MiJingItemInfo2
{
	MiJingItemInfo2() :item_id(0), num(0), is_bind(0), price_type(PRICE_TYPE_0), price(0), buy_times(0), discount(0) {}
	enum PRICE_TYPE
	{
		PRICE_TYPE_0 = 0,//元宝
		PRICE_TYPE_2 = 2,//铜钱
...
struct MiJingShopInfo2
{
	MiJingShopInfo2() {}

	std::map<int, MiJingItemInfo2> shop_info;
};

class SpecialLogicMiJingQiWenFB2 : public SpecialLogic
{
public:
	SpecialLogicMiJingQiWenFB2(Scene* scene);
	virtual ~SpecialLogicMiJingQiWenFB2();
	...
	virtual int GetMineMonsterBattleMode() { return BATTLE_MODE_RA_MIJINGQIWEN_2; }
	...
	void SetInfo(int layer, int level_seq, Posi transport_pos, std::vector<new_Event_NPC_cfg2> * has_create_npc_vec = NULL, std::map<int, std::vector<ItemConfigData> >* item_list = NULL);
...
private:
	...
	void RoleAloneGetRewardByCfg(Role *role, const MiJingQiWenFbNPCRewardCfg2 * npc_cfg, ObjID *targer_obj = NULL);
	void RoleTeamGetRewardByCfg(Role *role, const MiJingQiWenFbNPCRewardCfg2 * npc_cfg, ObjID *targer_obj = NULL);
	...
	//已创建过的NPC列表
	std::vector<new_Event_NPC_cfg2> m_has_create_npc_vec;

	std::map<int, std::vector<ItemConfigData> > m_item_list;					//暂存一些会掉落图鉴卡的奖励

	MiJingShopInfo2 m_shop_info;//商店信息
	std::map<UserID, BuyList2> scene_user_shop_map;//玩家商店信息

	std::map<ObjID, Protocol::MiJingQuestion2> m_question;
