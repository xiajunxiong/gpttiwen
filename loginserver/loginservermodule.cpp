...
public:
	...
	virtual void OnAccept(Port listen_port, NetID netid, IP ip, Port port)
	{
		...
		if (listen_port == m_server->m_battle_server_listen_port)
		{
			m_server->m_log.printf(LL_INFO, "BattleServer Accecpt.");
			LoginServer::Instance().OnBattleServerAccept(listen_port, netid, ip, port);
			return;
		}
	}
...
{
	{
		//printf("stop begin:%u", (unsigned int)EngineAdapter::Instance().Time());

		//printf("stop end:%u", (unsigned int)EngineAdapter::Instance().Time());
	}
...
int LoginServerModule::Start()
{
	...
	{
		m_log.printf(LL_ERROR, "Login Server On Start, Config [%s] Read Failed!", (ROOT / "ConfigDir").c_str());
	...
	if (!config->GetValue(ROOT / LOGIN_MODULE / "AllowIPPrefix", &allow_ip_prefix))
	{
		m_log.printf(LL_ERROR, "Login Server On Start, Config [%s] Read Failed!", (ROOT / LOGIN_MODULE / "AllowIPPrefix").c_str());
	...
	// 名字库配置路径
	std::string name_config_dir = config_dir + "/serverconfig/";

	std::string appearance_cfg_dir = config_dir + "/gameworld/logicconfig/";
	std::string agent_cfg_dir = config_dir + "/gameworld/logicconfig/";
	...
	// 为battleserver开listen端口
	if (!ListenForBattleServer())
	{
		this->StopServer();
	...
	if (!NameConfig::Instance()->Init(name_config_dir, configname, &err))
	{
		m_log.printf(LL_ERROR, "NameConfig Return Fail! Error At:%s", err.c_str());
	...
	if (!SPID_CONFIG->Init(config_dir + "agent_adapt.xml", &err))
	{
		m_log.printf(LL_ERROR, err.c_str());
		m_log.printf(LL_ERROR, "Login Server On Start, SpidConfig::Init return FALSE.");
	...
	if (!SPID_CONFIG->Init_1(agent_cfg_dir + "agent_adapt.xml", &err))
	{
		m_log.printf(LL_ERROR, err.c_str());
		m_log.printf(LL_ERROR, "Login Server On Start, SpidConfig::Init_1 return FALSE.");
	...
	if (!AppearanceConfig::Instance().Init(appearance_cfg_dir, "image_change.xml", &err))
	{
		m_log.printf(LL_ERROR, err.c_str());
		m_log.printf(LL_ERROR, "Login Server On Start, AppearanceConfig::Init return FALSE.");
...
{
	InternalComm::Instance().CloseAllDBSession(m_rmi);
...
bool LoginServerModule::ConnectToDB()
{
	...
	DBSession db_name_session(name_session, name_db_server_ip, name_db_server_port);

	m_log.printf(LL_MAINTANCE, "Connect Name DBLayer[%s:%d] Suc!", name_db_server_ip.c_str(), (int)name_db_server_port);

	InternalComm::Instance().SetDBSession(db_accounter_session, role_db_session_list, role_db_num, DBSession(), db_system_session, db_name_session);
...
bool LoginServerModule::ListenForGameServer()
{
	...
	if (!EngineAdapter::Instance().ConfigGetValue(ROOT / LOGIN_MODULE / "InnerComm" / "ForGameServer" / "Backlog", &inner_backlog))
	{
		m_log.printf(LL_MAINTANCE, "ListenForGameServer Config [%s] Read Failed!!!", (ROOT / LOGIN_MODULE / "InnerComm" / "ForGameServer" / "Backlog").c_str());
		return false;
	}
	...
	if (!ret)
	{
		m_log.printf(LL_MAINTANCE, "LoginServer GS NetListen return FAIL! port[%d]", m_gs_listen_port);
		return false;
	}
...
bool LoginServerModule::ListenForGateWay()
{
	...
	int gw_backlog = 64;
	if (!EngineAdapter::Instance().ConfigGetValue(ROOT / LOGIN_MODULE / "InnerComm" / "ForGateWay" / "Backlog", &gw_backlog))
	{
		m_log.printf(LL_MAINTANCE, "ListenForGateWay Config [%s] Read Failed!!!", (ROOT / LOGIN_MODULE / "InnerComm" / "ForGateWay" / "Backlog").c_str());
		return false;
	}
	...
	if (!ret)
	{
		m_log.printf(LL_MAINTANCE, "LoginServer GateWay NetListen return FAIL! port[%d]", m_gateway_listen_port);
		return false;
	}
...
bool LoginServerModule::ListenForBattleServer()
{
	m_battle_server_listen_port = ServerConfigPool::Instance().common_config.ipconfigs.login_server_addr.inner_connect_addr.for_battle.port;

	int gw_backlog = 64;
	if (!EngineAdapter::Instance().ConfigGetValue(ROOT / LOGIN_MODULE / "InnerComm" / "ForGateWay" / "Backlog", &gw_backlog))
	{
		m_log.printf(LL_MAINTANCE, "ListenForBattleServer Config [%s] Read Failed!!!", (ROOT / LOGIN_MODULE / "InnerComm" / "ForGateWay" / "Backlog").c_str());
		return false;
	}
	
	bool ret = m_internal_network->Listen(m_battle_server_listen_port, gw_backlog);
	if (!ret)
	{
		m_log.printf(LL_MAINTANCE, "LoginServer for Battle NetListen return FAIL! port[%d]", m_battle_server_listen_port);
		return false;
	}
	m_log.printf(LL_MAINTANCE, "LoginServer for Battle Listen[%d].", m_battle_server_listen_port);
...
bool LoginServerModule::ListenForUser()
{
	...
	int backlog = 64;
	if (!EngineAdapter::Instance().ConfigGetValue(ROOT / LOGIN_MODULE / "ForUser" / "Backlog", &backlog))
	{
		m_log.printf(LL_MAINTANCE, "ListenForUser Config [%s] Read Failed!!", (ROOT / LOGIN_MODULE / "ForUser" / "Backlog").c_str());
		return false;
	}
	...
	if (!ret)
	{
		m_log.printf(LL_MAINTANCE, "NetListen return FAIL! port[%d]", listen_port);
		return false;
	}
...
bool LoginServerModule::IsAllDataLoadFinish()
{
	return true;
}
