...
namespace wgprotocal
{
	enum _MsgType
	{
		...
		MT_GATE_REGISTER_TO_GAME,					// 网关注册到游戏服
		MT_GATE_REGISTER_TO_BATTLE,					// 网关注册到战斗服
		MT_BATTLE_NET_SEND_MSG,						// 战斗服给用户发消息
	};
	...
	struct WBRegisterToBattle
	{
		WBRegisterToBattle() : header(MT_GATE_REGISTER_TO_BATTLE) {}
		MessageHeader header;

		int gate_index;
	};

	struct WGRegisterToGame
	{
		WGRegisterToGame() : header(MT_GATE_REGISTER_TO_GAME) {}
		MessageHeader header;

		char hostname_for_user[64];		
		unsigned short port_for_user;
		unsigned short gate_index;
	};

	struct BWNetSendMsg
	{
		BWNetSendMsg() : header(MT_BATTLE_NET_SEND_MSG) {}
		MessageHeader header;

		int role_id;		// 检查nei_id是否对应role_id，不对应则放弃发送
		// 后面紧更着消息内容
	};
