...
int CMysqlUtility::ExecSelect(TMysqlStatement *pstMysqlStat, int* iErrNo, char *sErrMsg, int iErrMsgBufferSize)
{
	if (!pstMysqlStat)
	{
		return -100;
	...
	{
		SNPRINTF(sErrMsg, iErrMsgBufferSize, "iQueryType != SQL_TYPE_SELECT");
		return -1;
	}

	if ((pstMysqlStat->szSQLString[0] != 's') && (pstMysqlStat->szSQLString[0] != 'S'))
	{
		SNPRINTF(sErrMsg, iErrMsgBufferSize, "QueryType=1, But SQL is not select");
		return -2;
	}
	...
	if ((pstMysqlStat->pstCurMysqlConn == NULL) || (pstMysqlStat->pstCurMysqlConn->iDBConnect == 0))
	{
		SNPRINTF(sErrMsg, iErrMsgBufferSize, "Has Not Connect to mysql");
		return -3;
	}
	...
	if (iRet != 0)
	{
		...
		*iErrNo = mysql_errno(&(pstMysqlStat->pstCurMysqlConn->stMysql));
		SNPRINTF(sErrMsg, iErrMsgBufferSize, "Fail To Execute SQL: %s", mysql_error(&(pstMysqlStat->pstCurMysqlConn->stMysql)));
		return -4;
	}
...
int CMysqlUtility::SetCharSet(TMysqlStatement *pstMysqlStat, int* iErrNo,char *sErrMsg, char *pcCharSet /*= "utf8"*/)
{
	if (!pstMysqlStat)
	{
		return -100;
