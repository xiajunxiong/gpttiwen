...
{
	bool ret = GetValue("IsDelayLoad", &IsDelayLoad)
...
{
	bool ret = SetValue("IsDelayLoad", &IsDelayLoad)
...
bool LoginServerInnerAddr::read()
{
	bool ret = GetValue("Server_Ip", &server_ip)
		...
		&& GetValue("For_Battle", (ConfigElementBase*)&for_battle);
	return ret;
}
bool LoginServerInnerAddr::write()
{
	bool ret = SetValue("Server_Ip", &server_ip)
		...
		&& SetValue("For_Battle", (ConfigElementBase*)&for_battle);
	return ret;
}
...
bool ServerTimeOut::read()
{
	bool ret = GetValue("IS_Check_TimeOut",&is_check_timeout)
		...
		&& GetValue("BattleServer_Heartbeat_DeadTime_MS", &battle_server_heartbeat_dead_time_ms);
	return ret;
}
...
bool ServerTimeOut::write()
{
	bool ret = SetValue("IS_Check_TimeOut",&is_check_timeout)
		...
		&& SetValue("BattleServer_Heartbeat_DeadTime_MS",&battle_server_heartbeat_dead_time_ms);
	return ret;
}
...
bool CommonConfig::read_table()
{
	bool ret = GetValue("IPConfig", (ConfigElementBase*)&db_tables);

	return ret;
}

bool CommonConfig::LoadConfig(const std::string &config_path, char *err_info)
{
	std::string new_path = config_path + "/commonconfig.xml";
	const char * filename = new_path.c_str();
	// 加载配置
	...
	std::string table_path = config_path + "/commonconfig_table.xml";
	const char * table_filename = table_path.c_str();
	// 加载配置
	if (!m_document.LoadFile(table_filename) || 0 == m_document.FirstChildElement("config") || 0 == m_document.FirstChildElement("config")->FirstChildElement())
	{
		sprintf(err_info, "Load xml config %s fail.%s", table_filename, m_document.ErrorDesc());
		return false;
		//TiXmlElement tmp_root("config");
		//m_document.InsertEndChild(tmp_root);
		//m_document.SaveFile();
	...
	TiXmlElement *root2 = m_document.FirstChildElement("config");
	SetElement(root2);
...
bool BattleServerAddr::read()
{
	bool ret = GetValue("Addr", (ConfigElementBase*)&addr);
	return ret;
}

bool BattleServerAddr::write()
{
	bool ret = SetValue("Addr", (ConfigElementBase*)&addr);
	return ret;
}

bool DBTableServerAddr::read()
{
...
bool DBTablesConfig::read()
{
	bool ret = GetValue("DB_Server_Addr", (ConfigElementBase*)&db_server_addr);
	return ret;
}

bool DBTablesConfig::write()
{
	bool ret = SetValue("DB_Server_Addr", (ConfigElementBase*)&db_server_addr);
	return ret;
}
