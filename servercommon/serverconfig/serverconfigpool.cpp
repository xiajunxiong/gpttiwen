...
{
	if (!common_config.LoadConfig((config_path+"serverconfig").c_str(),err_info))
	{
		return false;
	}
...
{
	for (int i=0;i<common_config.db_tables.db_server_addr.NoCacheTables.TableCount && i<common_config.db_tables.db_server_addr.NoCacheTables.MaxTableNameCount;++i)
	{
		table_list.push_back(common_config.db_tables.db_server_addr.NoCacheTables.TableNames[i]);
	}
	return true;
}
bool ServerConfigPool::GetLocalDBNoDelayLoadTables(std::vector<std::string> &table_list)
{
	for (int i=0;i<common_config.db_tables.db_server_addr.NoDelayTables.TableCount && i<common_config.db_tables.db_server_addr.NoDelayTables.MaxTableNameCount;++i)
	{
		table_list.push_back(common_config.db_tables.db_server_addr.NoDelayTables.TableNames[i]);
	}
	return true;
}
