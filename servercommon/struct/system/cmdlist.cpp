...
bool CmdList::Serialize(TLVSerializer &outstream) const
{
	...
	{
		ret = outstream.Push(cmd_list[i].cmd_id) && 
			outstream.Push(cmd_list[i].creator.c_str()) && 
			outstream.Push(cmd_list[i].cmd.c_str()) &&
			outstream.Push(cmd_list[i].opid);
		if (!ret) return false;
	}
...
bool CmdList::Unserialize(TLVUnserializer &instream)
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		ret = instream.Pop(&cmd_list[i].cmd_id) &&
			instream.Pop(&creator_tmp) &&
			instream.Pop(&cmd_tmp) &&
			instream.Pop(&cmd_list[i].opid);
		if (!ret) return false;
